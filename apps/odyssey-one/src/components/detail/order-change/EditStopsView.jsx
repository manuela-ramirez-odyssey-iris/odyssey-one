...
import {
  Alert, Badge, Button, HeaderStrip, ModalFooter, SubAccordion, TitleSubtitle, ButtonToggle,
} from '@odyssey/ui'
...
export default function EditStopsView({ stops, consolidation, orders, orderChange, summary, onApprove, onCancel }) {
  // ponytail: initSandbox reruns if the parent re-renders with new `stops` —
  // acceptable, the parent mounts this once per edit session.
  ...
  const metric = (value, changed) => changed
    ? <Badge variant="purple" leftIcon={<TriangleAlert {...ICON_MD} aria-hidden="true" />}>{value}</Badge>
    : value
  ...
  return (
    <div className="edit-stops">
      ...
      >
        <div className="edit-stops__head">
          <div className="edit-stops__metrics">
            ...
            <TitleSubtitle subtitle="Distance" title={metric(distance, distanceChanged)} />
            <TitleSubtitle subtitle="Gross Weight" title={metric(curTotals.grossWeight, weightChanged)} />
            <TitleSubtitle subtitle="Volume" title={metric(curTotals.volume, volumeChanged)} />
          </div>
          <div className="edit-stops__head-actions">
            <Button variant="secondary" onClick={() => setModal('planning')}>View Planning Dates</Button>
            {routingDisabled ? (
              <TooltipTrigger tooltipProps={{ groups: [{ content: routable ? 'Read-only in Prior view' : 'Place every P? / D? stop first' }] }}>
        ...
        <div className="edit-stops__body">
          <div className="edit-stops__rail">
            {displayStops.map((s, i) => {
              ...
              return (
                <div className="edit-stops__card" key={s.key}>
                  <HeaderStrip
                    title={`Stop ${i + 1}`}
                    badge={(
                      <>
      ...
      {modal === 'discard' && (
        <DiscardChangesModal
          onDiscard={() => { setModal(null); onCancel?.() }}

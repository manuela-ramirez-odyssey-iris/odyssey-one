...
export const props = [
  ...
  { name: 'highlightRowId', type: 'string | null', desc: 'S155: the row whose TanStack `row.id` matches gets `data-highlight` on its <tr> (sibling of `data-selected`), which CSS turns into a one-shot 2.4s fade from --carolina-blue-100 back to the cell background — the "here is what you just created" flash after a create/apply, paired with a consumer-side pin of that row to the top of page 1. Purely visual and self-retiring (animation only, no persistent fill), and `prefers-reduced-motion: reduce` drops the animation entirely. Default null → no row highlighted.' },
...
//   sticky: 'left' | 'right' → pinned column: 'right' = the action column (edge resize
//                           grip logic is right-only), 'left' = a leading system column
//                           (e.g. the consolidate-mode select column, S155). Renders
//                           `odyssey-table__cell--sticky-left` / `--sticky-right`.

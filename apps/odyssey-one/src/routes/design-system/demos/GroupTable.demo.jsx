...
export const props = [
  ...
  { name: 'header', type: '{ title, icon?, badge?, trail? }', desc: 'Optional 48px strip above the column-header row (Figma 4183:773 "Header" frame): icon + bold title (+ optional badge beside it) left, trailing slot right. Presence renders the strip; omit it (the default) for the unchanged, released look. `icon` is a caller-supplied node (e.g. a lucide element) — the component hardcodes none. The table is `aria-labelledby` the title. (Figma: Group header BOOLEAN.) The strip itself is an INSTANCE of the standalone HeaderStrip master (5530:1140) — its `Title` / `Show icon` / `Icon` / `Show badge` / `Show trail` properties are exposed straight through on GroupTable instances that carry the strip. `badge` sits INSIDE the title group, beside the text it qualifies, never with the trail.' },
...
export const tokens = [
  ...
  { token: '--border-default', resolves: 'DSN/300', usage: 'detailSections: the 1px seam between SIBLING nested tables — on the 2nd and later only, since the group row above the first already carries one. A step darker than the --border-subtle hairlines INSIDE a table: the seam divides two independent tables, the hairlines divide rows within one' },

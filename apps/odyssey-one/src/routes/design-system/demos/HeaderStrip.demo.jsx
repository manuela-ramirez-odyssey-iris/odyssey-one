...
export const props = [
  ...
  { name: 'badge', type: 'node', desc: 'Optional Badge (or any node) rendered immediately AFTER the title, inside the same group — so it stays glued to the text it qualifies rather than drifting toward the trail. Figma pairs `Show badge` with a `Badge` instance-swap scoped to the Badge set, so the instance panel offers the variants and nothing else; in code the variant is simply a property of the Badge you pass, never a prop here.' },

...
export const props = [
  ...
  { name: 'badge', type: 'node', desc: 'Optional Badge (or any node) rendered immediately AFTER the title, inside the same group — so it stays glued to the text it qualifies rather than drifting toward the trail. Figma gates it with `Show badge` and EXPOSES the nested Badge, so its Variant dropdown appears on every HeaderStrip instance (an instance-swap cannot do this — its picker lists components, not variants). In code the variant is simply a property of the Badge you pass, never a prop here.' },

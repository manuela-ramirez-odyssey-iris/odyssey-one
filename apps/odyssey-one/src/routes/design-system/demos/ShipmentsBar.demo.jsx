...
export const props = [
  ...
  { name: 'tabs[].trailing', type: 'node', desc: 'Optional ReactNode rendered INLINE after the tab label (.shipments-bar__tab-trailing). For a live marker that belongs with the text — the SpotBid tab wears LiveBidDot here while a quote is open (user, 2026-09-07).' },
  { name: 'tabs[].indicators', type: 'node', desc: 'Optional ReactNode FLOATING over the tab\'s top-right corner and past the bar\'s top edge (.shipments-bar__tab-indicators, absolute top -7 / right -6 — the same float as the GlobalSearch Filters badge). Wrap each glyph in .shipments-bar__tab-alert (24px white circle, --shadow-sm, 20px glyph; a --purple modifier colours the glyph) to read as an alert. Small status glyphs that signal something about the tab\'s CONTENT — a generic per-tab slot, available on EVERY tab, not a Tender feature (user, 2026-09-07). First consumer: the Shipments Tender tab (S140) — a purple package for a pending order change, else an orange list-x for dropped carriers; one glyph at a time, because the order change blocks the whole tab. A node, so one glyph or several. (Figma: ShipmentsBarTab `Show indicators` BOOLEAN + `Indicator` INSTANCE_SWAP, placeholder-16 by default — added 2026-09-07.)' },

...
export const props = [
  ...
  { name: 'tabs[].indicators', type: 'node', desc: 'Optional ReactNode rendered INLINE after the tab label (.shipments-bar__tab-indicators, inline-flex, spacing-1 gap). Small status signals about the tab\'s CONTENT — a generic per-tab slot on EVERY tab, not a Tender feature. Consumers (S140): the Tender tab shows a purple icon-only Badge (OctagonAlert) AND turns its own label purple while an order change is pending — the one thing that BLOCKS the tab; the SpotBid tab shows a pulsing live dot while its quote is open. A floating corner-alert variant was tried and dropped the same day (user, 2026-09-07). (Figma: ShipmentsBarTab `Show indicators` BOOLEAN + `Indicator` INSTANCE_SWAP, placeholder-16 by default.)' },

...
export const props = [
  ...
  { name: 'onSelect', type: '(e) => void', desc: "Makes the WHOLE card the select target and gives it `role=\"button\"`, a tab stop, `aria-pressed` and Enter/Space. A click or key that lands on an interactive DESCENDANT belongs to that control instead. CONTRACT: pass this only on a card with no interactive children — a button inside a button is invalid HTML and an AT trap, which is why the Shipments PGI/PGR cards carry no footer Go-to link. Becoming selected replays the entry animation (donut grow-in + count-up), the same false→true edge WidgetMini uses. Code-only extension (Pending Figma Sync)." },

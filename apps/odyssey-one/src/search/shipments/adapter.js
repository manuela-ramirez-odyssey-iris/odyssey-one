...
import {
  toItem as coreToItem, nextProgressionGroup as coreNextGroup,
  dateItems as coreDateItems, DATE_LIKE, parseDatePartial,
} from '../adapter-core'

// ── Dates (Case 12, GS-22) ──────────────────────────────────────────────────
// The machinery itself is domain-agnostic and lives in ../adapter-core.js
// (moved there S131, when Orders needed the identical behaviour). This file
// supplies only WHICH attributes are dates, and re-exports the two names its
// existing importers (liveAdapter, ShipmentsGlobalSearch, tests) already use.
const DATE_ATTRS = SHIPMENTS_ATTRIBUTES.filter((a) => a.match === 'date')
export { DATE_LIKE, parseDatePartial }
const dateItems = (query) => coreDateItems(query, DATE_ATTRS)

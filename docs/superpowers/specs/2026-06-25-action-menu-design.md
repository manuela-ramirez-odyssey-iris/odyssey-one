...
## MenuRow
**No `MenuRow.jsx` change needed.** `MenuRow` already supports `disabled` (skips `onClick`, sets `data-disabled`/`aria-disabled`, with `.menu-row[data-disabled]` muted styling) and a `className` pass-through, and spreads `...rest` onto its `<div>`. So `ActionMenu` passes `disabled` straight through, expresses `danger` via `className="menu-row--danger"` (a new CSS rule), and adds menu a11y (`role="menuitem"`, `tabIndex`, Enter/Space activation, since `MenuRow` is a `<div>`, not a `<button>`) through `...rest`.

...
1. **Write (or update) `<Component>.demo.jsx`.** The file must export:
   ...
   **PORTED redefined (2026-07-05, user decision):** `ported: true` is set in BOTH DSMs **as soon as the twin is generated + machine-verified** (parity-lint + builds + specs) — automatically at the end of the batch port. Batch approval therefore flows straight through the port with no intermediate review gate.
   ...
   **Asymmetric staging badges (2026-07-05, user decision):** the two explorers render staging differently.
   ...
   - **ALWAYS output the library PR's URL when a version is pushed (HARD RULE, user 2026-09-09).**
     Not the PR number, not "the PR is open" — the clickable link, in the reply that reports the
     push, every time. This includes folding a late component into an already-open PR at an
     unchanged version: that is still a version push, and the link goes out again with it. The
     release is only *reported* when the user can reach the thing being released; a release
     they have to ask for the link to is not finished. `gh pr create` prints the URL and
     `gh pr view <n> --json url` retrieves it later — there is no excuse for omitting it.
...
- [ ] **Do NOT set `meta.version` / `meta.createdVersion` here.** The component's library version isn't known until release — `/port-to-angular` stamps both fields on both demos when it bumps `@oneodyssey/ui` (Phase 5). The React demo carries neither while it sits in the Normalizing tab. Semantics: `createdVersion` = the release the component FIRST shipped in — stamped **once**, never bumped afterward (drives the DSM "Created in" dropdown); `version` = the release it was created or **last changed** in — advances on every batch that touches the component (drives the card version badge).

...
1. **Organisms Home uses — ONLY these (Manuela's scope).** Port to Angular via `/port-to-angular`: **Opus** generation, **verbatim-React demos** (same sample data/content), and **puppeteer screenshot + measurement parity before every GATE B** (harness at `/tmp/dsm-measure` — `shots.js`, `focus-measure.js`). Dependency order (all molecule deps now ported): **Navbar** (unblocked — GlobalSearch ✓ / LeadNav ✓ / TrailNav ✓) · **Widget** (Tier 4 — IntersectionObserver + rAF count-up) · **WidgetsLeftMenu** (render-prop → `@ContentChild TemplateRef`) → **WidgetVariantPicker** (needs Widget) → **ModalLarge** (its Home/demo content composes WidgetVariantPicker). Watch the **wrapper-host trap** (rules React applies to the component root go on the Angular `<odyssey-*>` HOST, the flex child — hit on FieldSelect + GlobalSearch). See `project_angular_organisms_home_scope`. **Molecules complete (18/20)** — EntityChip (deprecating) + Cell (CSS table-skin, not a component) intentionally excluded.
2. **Push odyssey-one** — S60's tracker + progress commits are local; push them. Angular stays local-only on `build/angular-dsm`.
3. **Deliver the library + DSM zip** — `Shipments/odyssey-ui-delivery-2026-06-17.zip` (README serve-path fixed S59) to Cognizant; re-package at the current **35-component** state. Owed.

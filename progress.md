...
1. **Product counter bug (user-blocking):** "0 products added" doesn't update in the user's browser — works headless on all three commit paths (dropdown pick, type+click-away, type+Enter fixed this session). Debug LIVE with the user: stale dev server first suspect, then capture their exact flow.
2. **Product-field lookup research (user-stated focus):** how data looks when loaded in the comboboxes — close the 4 format gaps: Product ID dropdown row anatomy (LINX-9875), Harmonized Code lookup source/format (struck rule, LINX-9874), real Handling Unit catalog (LINX-9879 / legacy Packaging ID), Product Class option source. Route: targeted Jira `*all` fetch + ask user for QA screenshots of those dropdowns (the carrier/org screenshot route settled formats faster than Jira).
3. **Orders/Create remainder:** "Add More Details" row expansion (LINX-8131 Product Details), dropdown footer "Don't see your address? + Add manually" question (Efrain), TypeaheadSelect → ComboBox migration for Products/Special Services lookups, LINX-8042 address cascade, appointment propagation (13869), Edit-flow live gap.
4. **Batch re-approval + Angular port batch (growing):** ModalMedium, Paginator, DataTable, ComboBox (36px + panelError + row passthrough + Enter-commit + error placeholder — Angular twin + Code Connect publish owed), ShipmentsBar (Figma sync owed), DatePicker/TimePicker/CalendarPicker, Accordion, S90 error-state components, **`--spacing-9` token → Angular tokens file**. Angular DSM flags staged locally on `port/s76-search-batch` (no push).
5. **End-of-Orders reseed** (permission-gated, one motion with deploy): work the `vault/10-domains/orders/db-update-ledger.md` shopping list (equipment pool swap, freight-term codes, EXTRA_ORGS scope, product-line fields). Keep adding items as they accrue.
6. **DB slices 4–7** (lookup endpoints feed the create-order comboboxes — customer search, org-scoped equipment, carrier, org-address). rtk trial evaluation (`rtk gain`). PR #12 watch (blocked on `gh auth login`).
7. Carried: S87 punch-list remainder, 3 real API gaps, Home 1x-title wrap, MiniMultiselect (Efrain), ActionMenu cell-click flicker, 9.3MB chunk code-split, `.worktrees` cleanup, MatchSimpleRow px diff, DesignSystemMap legacy refs. Orders S94 remainder: Export deploy-verify, Filter decision, live Submit/Cancel, Resolve→OIF (LINX-11137), location-sort semantics.

...
**THE RESEED-SCOPE AUDIT THAT BECAME A FULL SHIPPED MOTION — opener review found the one real gap (order-number format, never a ledger row), which grew into ledger row 10, a combined Neon reseed + prod deploy (user-gated, executed), the ComboBox disabled cycle through a full 0.9.1 release, a create-order UX round, and PR #13 unblocked with PR #12 discovered MERGED by Cognizant.** Tests: app **566/566** (+3), api **34/34** (+6), tools 9, Angular lib **979/979** + explorer 52/52. DB reseeded ×1 + deploy ×1 (both user-gated, one motion). Angular pushed ×3 (all user-approved: 0.9.1 release, CI fix, merge-integration).

- **Scope review (opener):** rows 1–9 verified correct against live probes; the "old format" the user saw was order numbers — never in scope (the `ABC1000NN` shape was S80 invention). Canon: LINX-9742/9279 — Order Number optional free text (≤150 chars, unique per owning org), blank → BE generates `orderNumber = orderId`. **Ledger row 10** (user final: **71% auto-numeric 13-digit zero-padded** mirroring the row-5 product-ID convention / **29% user free text w/ customer initials** `COV-91150` — ratio+shape flagged invented); **row 11** (per-order errors payload) verified **no-op** — S100 already closed the `?resolve=` refresh seam. Post-deploy "no Valtris numeric orders" scare root-caused as **lexicographic desc sort** (letters rank above digits; free-text rows stack first) — user accepted, no change; an orderId-based newest-first sort noted as the clean fix if it ever bothers.
- **The combined motion (subagent-driven, one reseed + one deploy):** generator → shared numeric id sequence 91000+, `String(orderId).padStart(13,'0')` autos / `PREFIX-<id>` user-provided, every row now carries `orderId` (5136 rows, 29.9% user, resolve fixture re-pinned `0000000091105`); write-path mock aligned (blank create → same padded form, `S26…NGW` gone); **preferences wire** — `GET/PUT /user-service/v1/preference` over the S92 `user_preferences` table + `useUserPreference` hook + ShipmentsRoute persists ColumnPanel presets (`shipments.columnPresets`, save-on-commit only); 4 sso-mock personas added to seed-users (FK fix — u1–u4 didn't exist in the users table); **resolve mode on real `disabled` props** end-to-end (resolveFieldProps returns `disabled:false` for pool fields; opacity-0.6 + pointer-events blanket CSS deleted; focus-bounce guard obsolete → removed; ProductGrid/RepeatableRows/MeasureField gained `disabled` props). Reseed 9m59s: 10k shipments / 24,056 orders; live probes green incl. a `u4` preference PUT.
- **ComboBox disabled cycle (full /normalize → 0.9.1):** Figma set 4715:6142 gained `State=Disabled` × Search|Select (12→14 variants; user toned in-bar icons DSN/500→400 + republished); code delta was only the select chevron (`--text-placeholder` + not-allowed when disabled, both stacks); Code Connect State mappings completed (error/validated had never been mapped); DSM demos gained disabled toggle + props/tokens rows in both stacks; angular-porter twin update w/ a computed-style guard spec. Full staging cycle same session (demote → approve → port → **user final approval → release 0.9.1**, both repos pushed, rides PR #13). Efrain flag: FormField Figma Disabled text DSN/900 vs code `--text-tertiary` (pre-existing drift).
- **Create-order UX round (all Chrome-measured):** ColumnPanel **push model** (was a fixed overlay hiding the grid — page shifts left like Shipments; breakout math shrinks via `--co-panel-w`); page horizontal padding 32→24px (scoped negative margin, shell untouched); breakout side gap 24→20px; `.co-addmore` 24px both states (was 48 expanded); **ColumnPanel live-apply** (user rule: drag/toggle applies to the table instantly, Save only persists, Cancel/Back/discard revert via `revertAppliedColumns`) — shared component, lands everywhere.
- **PR #13 unblocked + the Normalizing-tab mystery:** Build Check red on a stale explorer spec (hardcoded `0.8.0` version chip — 0.9.x release tripped it; porter agents don't run explorer app specs) → assertion now derives from `projects/odyssey-ui/package.json`, stale-proof; branch had diverged — **Balaji merged main in, revealing PR #12 (S89/0.8.0) MERGED by Cognizant**; merged + pushed, Build Check GREEN — PR #13 now only awaits Cognizant review. The "ComboBox still NORMALIZING in Angular DSM" report (user hit it 3×): repo was correct on every branch — a **zombie `ng serve` from July 17** held port 4200 and served the pre-release build to every browser; killed both processes + cache, fresh serve verified in Chrome (Normalizing 0, v0.9.1). Lesson: stale DSM = check `lsof -i :4200` for the *process*, not just `.angular/cache`.

---

## What's Next (S104)

1. **GLOBAL SEARCH → the new DB (user priority).** Migrate GlobalSearch off the in-memory/lazy index onto Neon. Explore the **HTTP `QUERY` method** and design a genuinely scalable search architecture — efficient server-side under MANY concurrent users, not a per-client scan. **This is meant as a reference implementation to present to the dev team**, so it has to be defensible on its merits: indexing strategy, pagination, debounce/cancellation, and measured numbers. Canon lives in `vault/20-cross-cutting/global-search/`.
2. **DEPLOY IS OWED — the UI is NOT live.** The reseed IS live (prod reads Neon at runtime), so production currently shows the new data through the OLD modal. Blocker: `vercel --prod` ships the WORKING TREE, not HEAD, and the tree carries another session's `GroupTable` nested-table work (**NORMALIZING in both DSMs, Angular port owed**) plus `Spotboard.jsx`. Decide whether to ship the tree as-is or wait for GroupTable to reach a releasable state — the modal's UDF tab consumes GroupTable, so they can't be cleanly separated.
3. **Remaining `CST` hardcodes in the generator** — visible in the UI now that the dates beside them are correct: a Phoenix stop reads `Date … MST` but `Appointment: 09:00 CST`. Covers stop `appointmentTime`, order `pickupAppointment`/`deliveryAppointment`, and the `*TimeZoneCode` fields. Needs a reseed.
4. **Tracking follow-ups (TR-01…TR-08):** obtain the consolidation XML's `SubShipmentList` / `PickupSequence` / `DropoffSequence` (authoritative order↔stop binding, would supersede our inferred model); no LLD or Swagger exists for the tracking endpoint; `permalink` auth/TTL is security-relevant; `totalLoads: 0` unexplained; `relatedShipments` bidirectionality means `totalRecords` overstates real movements — dedupe before any KPI. A tracking-EVENT entity is a genuine gap (our `events` table is an audit log, different producer/lifecycle/volume).
5. **Tracking Link has no backing field anywhere** — renders `--` permanently until the contract carries a URL.
6. **`saveTender` replaces the whole option JSONB instead of merging** (carried from S102) — safe today because the UI always sends the complete object, but any partial write silently drops ~56 fields.
7. **Angular 0.10.0 remains committed LOCALLY ONLY**, awaiting explicit push approval; npm publish is Cognizant's; PR #13 merge watch continues.
8. **Efrain flags** (carried): charge-row delete affordance; `Field/height` not bound to the FormField master; FormField models neither `format` nor the chevron inset; Layout axis, uppercase text-style, selected-state convention; FormField Disabled DSN/900 vs `--text-tertiary`.
9. **Make the puppeteer verification a real `tools/` entry** — it earned its keep again this session (caught nothing broken, but it's the only thing that proves layout). Script currently lives in the session scratchpad.
10. Carried: Overflow Board (no canon exists), parked S98 `/analyze` intake, DB slices 4–7, `rtk gain` evaluation, S87 punch-list remainder, 3 real API gaps, Home 1x-title wrap, MiniMultiselect, ActionMenu cell-click flicker, 9.3MB chunk code-split, `.worktrees` cleanup, DesignSystemMap legacy refs, S94 Export deploy-verify + Filter decision + location-sort semantics, orderId-based newest-first sort.

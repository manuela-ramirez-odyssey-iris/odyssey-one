...
**THE ORDERS PER-TAB TABLE SHIPPED END-TO-END — full research arc (Jira + Figma + PO feedback) → plan → subagent-driven build (11 tasks, implementer + spec + quality review each) → live cutover (migration + reseed + deploy).** Tests: app **510 → 521/521**, api **22 → 25/25**, generator **3 → 4/4**, tsc + build clean. 3 prod deploys; DB reset + reseeded twice (all invariants green, 265 MB).

- **ModalMedium contract fixes (session opener):** footer buttons now spread via the shell's space-between — consumers pass fragments (DiscardSaveModal + DSM demo wrapper divs removed, dead `.co-modal-footer` CSS deleted); sizing → width auto 350–780px, height auto capped 90vh with the content slot scrolling past the cap. ModalMedium → NORMALIZING both DSMs. Memory `feedback_modified_component_back_to_normalizing` broadened: contract/demo-level changes demote even when the component source is untouched.
- **Orders research arc (the day's backbone) — two vault notes, both Fable-verified:** `vault/10-domains/orders/research/jira-orders-table-columns-2026-07-26.md` + `jira-create-order-sections-2026-07-26.md`. **KEY METHOD LESSON: Jira specs live in `customfield_10032` ("Acceptance Criteria"), NOT description** — first fetch pass missed everything; always fetch `*all`. Per-tab column ACs extracted (All 13-col wide set w/ inline strikethrough history, Draft lean 5-col, VE 4-col + Resolve→OIF), create-flow field specs (all 4 sections + Long-order sub-sections + equipment applicability matrix LINX-13893 + every verbatim validation string), confirmation pages (sync→async dual-message model), pagination spec (LINX-8091/9128: sizes 10–40 step 5, default 25), row-action rules (LINX-10233: Edit/Cancel Manual-only, Restore Cancelled-only), backend mid-migration to `om.draft_error_orders` (DDL captured).
- **Team-feedback layer:** Ramesh's tracker + PO punch-list xlsx processed via MarkItDown (inbox) — full October-scope Y/N list (Overview+3 tabs+View+Basic Filters+Edit/Cancel+whole Quick Order flow = IN; Long Order epic+Copy+Restore+Manage Columns+Custom Views+Advanced Filters+Home order widgets = OUT); 5 new feedback tickets fetched (13553 Owning Org→Customer rename, 12257 banner → "Fields marked with an asterisk (*) are required" per 2026-07-27 comment, 13563 remove section sub-text, 13869 Appointment propagation, 13833 Reference Type→dropdown, out of Oct). PO punch-list contradictions logged (checkboxes: PO says remove — user ruled DROP).
- **Figma verification (3 frames, re-fetched + independently confirmed):** All = 15 flat columns w/ 3-line location cells ("Shipper Location"/"Destination Location", no sub-column headers), badge variants (Hazmat=amber "Harmat" typo in file, New=blue, Ready for Planning=green, Rating/Routing Failed=red); VE = direct Resolve button (disabled unless status Ready — correlation confirmed vs mock), duplicate "Errors Count" column = mock artifact; Draft = "Create/Create By" label typos flagged. No toolbar/tabs/pagination in any frame.
- **THE BUILD (plan `docs/superpowers/plans/2026-07-27-orders-table-per-tab-columns.md`, 12 tasks):** per-tab column models (`ordersColumns.jsx` — three sets + badge maps + per-row action rules), OrdersTable tab-driven (checkboxes REMOVED, VE Resolve column, controlled sorting), route (per-tab default sorts, Submit/Cancel mock mutations + Jira-copy ModalMedium confirms, Resolve no-op pending LINX-11137), toolbar (sort-toggle removed, Filters disabled pending global-search decision, Export button), Export to Excel (ModalMedium flow, 25k cap w/ exact spec string, per-tab shapes, lazy `xlsx` chunk), **Paginator standardized ALL domains** (10–40 step 5, default 25 → NORMALIZING both DSMs), generator + migration 002 + seed + API projection/SORT_MAP. Decision log ORD-04.
- **Hazardous done RIGHT (user catch):** initial independent random draw replaced with derivation from product lines (LINX-12102 — order hazardous ⇔ ≥1 hazmat line, matching shipment-detail hazmat exactly); pool rebalanced 12→2 flagged products → **~19% of orders hazardous** (was 79%).
- **Live cutover + incidents:** migration → reset → reseed → deploy. Two ops lessons: (1) a subagent's backgrounded 12-min seed **dies when the agent's turn ends** → orphaned partial insert → PK collisions; recovery = reset ritual re-run from the main session; (2) deploy raced ahead of the agent — deployed old API code returned `hazardous: null` (looked like a UI bug; was the missing deploy). Final state verified live: 22,822 orders, hazardous 19/100, all verify-seed invariants green.
- **Tender tab fixed + deployed:** S93's ShipmentDetailsModal extraction left a stray `setIsDetailModalOpen(false)` in RoutingGuideTab's mount effect → ReferenceError crashed the pane (both modes — NOT the DB migration). One-line deletion, headless-verified, deployed with the pending S93 styling commits.
- **ComboBox (former SearchField) — full Figma-first cycle in-session:** Figma master renamed + converted to component set `4715:6142` w/ `Variant` axis Search|Select (Select = no leading magnifier, trailing 20px chevron-down), description "former SearchField"; code renamed w/ `SearchField` alias export kept, `variant='select'` (chevron rotates open, clear-X before chevron per FieldSelect convention), variant-aware placeholder default ('Select'/'Search', editable per instance); DSM demo renamed "(former SearchField)", NORMALIZING. Rationale: ARIA role name, and SearchField had outgrown its name. **Angular twin + Code Connect publish owed at batch close.** TypeaheadSelect (create-order) = first migration candidate (it lacks the lazy paged loading ComboBox has).
- Misc: DataTable non-driving sort arrows DSN-400 → **DSN-300** (rides the open NORMALIZING batch); `.env.local` runs live mode — use `VITE_API_MODE=mock npm run dev:odyssey-one` for mock data.
...
1. **Order Creation flow** (user-stated S95 focus) — implement against the October-scope feedback batch, all spec'd in `vault/10-domains/orders/research/jira-create-order-sections-2026-07-26.md`: renames (Owning Org→Customer LINX-13553 incl. error strings question, Consignor→Shipper 12255, Consignee→Destination 13899), banner → "Fields marked with an asterisk (*) are required" (12257, 2026-07-27 comment supersedes AC), remove section sub-text (13563), Hazardous order flag (12102, auto from lines, can't-uncheck rule), Appointment checkboxes (12095/13845, latest-date-gated; Efrain Figma done 07-24) + propagation (13869), equipment-based Product fields (13893 4-case matrix), Order# 150-char limit + dup-check, Consignor≠Consignee validation, ≥1-product gate. Also: migrate `TypeaheadSelect` onto **ComboBox variant='select'** (gains lazy paged loading).
2. **Batch re-approval + Angular port batch (growing):** ModalMedium, Paginator, DataTable (sort arrows DSN-300 + S93 features), ComboBox (former SearchField — Angular twin + Code Connect publish owed), ShipmentsBar (Figma master sync owed — id as link), + S90 error-state components. Angular DSM flags staged locally on `port/s76-search-batch` (no push).
3. **Orders remainder:** Export deploy-verify on all tabs live, sort-arrows deploy (committed, rides next deploy), Filter decision (halted — global search?), live Submit/Cancel endpoints + onError feedback, Resolve → OIF UI (LINX-11137), location-sort semantics mock (locationId) vs live (city) — decide before it matters. Flag to Efrain: "Create/Create By" Draft-tab labels, duplicate Errors Count column, Errors-Count plain-text vs colored circle.
4. **DB slices 4–7** (search endpoint, customers/lookup, first write, fake login + guest read-only) + Edit-flow live gap (`getDraft` live branch). rtk trial evaluation (`rtk gain`).
5. **PR #12 watch** — blocked on `gh auth login`. Then merge → Cognizant publishes 0.8.0 → branch cleanup → `domain-usage.json` regen.
6. Carried: S87 punch-list remainder, 3 real API gaps (checkbox `defaultChecked`, navbar `trailRef`, trail-nav `onMenuClick`), Home 1x-title wrap, MiniMultiselect (Efrain), ActionMenu cell-click flicker, 9.3MB chunk code-split (xlsx now lazy — main chunk unchanged), `.worktrees` cleanup, MatchSimpleRow px diff, `playground/DesignSystemMap.html` legacy SearchField refs (superseded file — clean or archive). `cell-tab-mapping.xlsx` = KEEP.
7. **Process rules:** Neon DB shared with prod — reseed+deploy one motion, migration FIRST. Subagent background jobs die with the agent's turn — long seeds run from the main session. Jira research: always fetch `*all` (AC lives in customfield_10032).

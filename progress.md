...
**THE PROGRESSIVE-SEARCH SESSION — seven behaviour cases shipped in mock (GS-14…GS-20), a root-cause discovery that reframed every symptom the user had reported, two research tracks, an architecture spec, and a 15-task execution plan. Three of my own calls were WRONG and had to be corrected mid-session, two of them because I inverted the user's words.** Tests: app **596 → 631** (+35). **Nothing deployed, nothing reseeded, no server code written** — the whole session landed in mock mode + docs. Next session executes the plan with subagents.

- **The root cause, found by measurement not by guessing.** The user reported search results "not showing as expected" and suspected the DB wiring. `.env.local` carries **`VITE_API_MODE=live`**, and the live API has **no search implementation at all**: `buildListQuery` never reads `filter.searchCriteria` (the client sends it, the SQL builder drops it), `buildCountsQuery` takes no criteria parameter, and `SORT_MAP` has no relevance entry so it silently falls back to `pickup_ts`. **Every symptom traced to that one gap** — table showing all rows, unfiltered tab counts, first rows matching nothing. Schema verified: **all 14 searchable columns already exist in Neon**, so nothing is missing from the seed — search can be wired with zero reseed. Everything built this session runs in the mock branch, which the user's session never executes.
- **Cases + decisions shipped (mock).** **GS-14** an untouched bar suggests nothing (suggestions became reactive; drill-forward survives) · **GS-15** a bare code resolves to WHAT IT IS — rows label themselves `Order #0000000091000` instead of being relabelled with the shipment number, and `FREE_TEXT_KEYS` widened 8 → 14 because **a pasted Pro/BOL, load, trailer or seal number returned ZERO results** (measured; the single most likely thing to paste) · **GS-16** the table inherits the preview's relevance order · **GS-18** the landing tab follows the preview's leading group · **GS-19** panel tabs are permanent · **GS-20** multi-code lists UNION. Cases 3–9 written up; **Case 10** (natural-language via an explicit "intelligent" button) logged as future scope with its seam reserved.
- **Three corrections, all mine.** (1) **GS-16 shipped DEAD.** The relevance sort sat behind `else if (!params.sortBy …)`, but `ShipmentsRoute` seeds `sorting` with a real column so `sortBy` is ALWAYS sent — the branch never executed in the app. **My test passed because it called `getShipmentErrorList` directly and omitted `sortBy`, exercising a path the product never takes.** Fixed with a `RELEVANCE_SORT` sentinel; the replacement tests send exactly what `listParams` builds AND assert the old params produce a different first row. (2) **GS-19 backwards** — read "top bars are always visible even when 0" as *so hide it too* and removed the PGI/PGR exemption; the user meant the opposite. Panel tabs are now permanent and the decision-8 fallback machinery is deleted. (3) **GS-20 backwards** — read the user's *"CODE123 & CODE001"* as boolean AND (all codes on the same row) when they meant the union of result sets. **In boolean search AND/OR describe conditions on ONE ROW, so "show A's results and B's results" is what logic calls OR** — a genuine naming trap, recorded. Measured both ways: a cross-row pro pair → **0 rows under AND, 4 under union**.
- **A test that proved nothing, caught by mutation.** The first preview-order test passed **with the relevance sort disabled** — the fixture happened to already be in the right order. Rewritten to derive a *discriminating* query (one whose exact match is provably NOT first in natural order) and mutation-checked: disabling the sort now fails 2 tests. **Lesson recorded in the case notes: a passing order-assertion is worthless unless the two orders provably differ.**
- **Research → spec.** **HTTP QUERY is now RFC 10008** (Proposed Standard, June 2026 — safe, idempotent, cacheable with the body in the cache key; Node ≥22 parses it, browsers send it) **but Vercel returns 400 for it** (vercel/next.js#77390, unresolved) and **CloudFront hard-rejects it** — which is our pending infra. Verdict: **document-QUERY, ship-POST**, endpoint QUERY-shaped in everything but the verb. Architecture: a narrow **`search_index` projection table** (`domain, entity_id, attr, value`) + three purpose-built indexes + a **tiered exact/prefix/contains UNION** with a TOTAL order, so preview≡table holds by construction rather than by two implementations agreeing (which they didn't). **Neon read replicas are compute-only and available on the FREE plan** — search structurally cannot saturate the primary. Escalation ladder written down (tune Postgres → ParadeDB `pg_search` on Neon → external engine) so nobody designs for Elasticsearch prematurely.
- **The user's buffer-zone instinct, confirmed and made an invariant** (spec §6a): while typing, only the narrow index is touched — ≤15 hit rows, single-digit KB; the wide rows are fetched only on click or commit. S79b decision 5 ("typing never filters the table") is therefore a **load rule**, not just UX.
- **Findings that reshaped the plan.** (1) **Pickup # is ORDER-level** — a customer-provided Order Header reference copied to the Load (user's Rovo research + our LLD reading agree), which is exactly why Jana couldn't find it in the shipments table. Since a shipment consolidates N orders it carries **N pickup numbers** → an ARRAY column, plus a "Find the shipment" search criterion **and** a Column-Arrangement entry (user). A latent bug falls out: `generate.mjs:566` emits the stop pickup number as a **coin flip** (`faker.datatype.boolean() ? … : null`), so half of all pickup stops render `--` despite being fully plumbed. (2) **The hardcode the user remembered was the STOPS TAB, not Tracking Link** — `generate.mjs:559`/`:602` emit a literal ` CST` for `appointmentTime` while the `scheduledDateTime` on the line ABOVE correctly derives per stop city and instant (S103), so a Denver stop shows `MDT` scheduled / `CST` appointment in the same grid; every stop on a multi-stop shipment also shares one appointment hour. **My component-level hardcode audit could never have found it — it lives in the generator and the component renders it faithfully. Lesson: audit the DATA path, not just the render path.** Tracking Link is a second, separate one. (3) **Appointment is timestamps, not booleans** — the Orders LLD carries `pickupAppointment`/`deliveryAppointment` + timezone codes, and maps **Late Pickup = `pickupAppointment`**, matching David Johns' *"that noon becomes the appointment"*. So the open question may not be "where does the flag go" but "is it a flag at all". (4) **LINX-13893 is an equipment applicability matrix** (4 cases driving which Product Information columns render), with two unreconciled conflicts: STCC struck-from-base vs live-for-Rail, and `TLF` in no case. (5) hazmat ≡ hazardous; our code splits by domain (`hazmat*` in Shipments, boolean `hazardous` in Orders).
- **Deliverables.** Spec `docs/superpowers/specs/2026-07-31-progressive-search-architecture-design.md` · plan `docs/superpowers/plans/2026-08-02-search-architecture-and-reseed-motion.md` (15 tasks, 4 tracks, TDD steps) · **ledger Round 2** in `vault/10-domains/orders/db-update-ledger.md` classified by what actually needs a reseed (4 data items) vs code only (4) · GS-14…GS-20 + Cases 3–10 in the global-search vault. Also closed a doc gap found while inventorying: **Case 8 had tests but no doc entry**, and Case 7 still read as current though GS-18 superseded its landing rule.
- **User rulings.** Full projection architecture now (not the direct-SQL bridge) · ONE combined motion (all code + generator changes, then a single reseed) · mock stays the case-discovery environment · **HALTED:** LINX-13893 columns and Appointment placement (user is asking) · **BLOCKED:** D1 — is the "Odyssey username" the email local-part? (recommended `amy.cook`; deviates from LINX-11663 "plain full names", so it needs a logged decision).
- **Next session: execute the plan with subagents.** Tasks 1–8 (the whole search architecture), Task 10 Steps 1–2 (Tracking Link), Task 10b (the stops `CST` fix) and Task 13 (order creation) are executable with **no answers needed**.

---

## Session 103 — July 30–31, 2026

**THE STOPS/ORDERS CORRECTION SESSION — the Shipment Details modal rebuilt to Jana's annotated spec, the stop↔order model corrected THREE times against progressively better evidence (my own over-correction reverted in the middle), the real Tracking payload analyzed into a new domain canon, timezones moved to IANA, and Neon reseeded + verified + browser-verified.** Tests: app **585 → 596** (+11), tools 2 (seed parser). DB reseeded ×1 (user-gated; first attempt killed mid-run and left the DB broken — see ops). **Nothing deployed** — UI changes are NOT live; the reseed IS.

- **ShipmentDetailsModal rebuilt to the 2026-07-30 annotated spec** (`vault/10-domains/shipments/shipment-details-modal-spec-2026-07-30.md`), **superseding the S102 Figma 1348:16364 four-section model**. `SummaryStrip` identifiers header (Buy Shipment · Pro/Booking # · Tracking Link · Rating Status · Sell Shipment) → § General Information (11 fields) → § Cost (the 7 user-named rows) → § Stops (summary; every stop links to `/orders/:orderId`, one link per order, wrapping) → § Customer Reference Values (order in col 1, its references flowing). **User Defined Fields behind its own tab**, `GroupTable` one group per order. Layout mimics `CostAllocationTab`: tab row on top, strip flush beneath (gap 0). **Footer removed entirely** (View Stops dropped); Routing Query (QCP) moved to the tab row trailing side, `size="sm"`. Content pinned 872px so tab switches don't resize the dialog; 32px bottom padding. `ModalMedium` deliberately NOT modified — the edge-to-edge header-slot idea was raised and dropped, so no normalize cycle.
- **Three mapper/contract additions:** `ShipmentDetailVM.ratingStatus` (generated and on the wire since S92 but **never read** by any mapper), `CostSummaryVM.directCost` (**summed across orders** — the wire has no shipment-level field; the old modal showed `orders[0]`, wrong on every multi-order shipment), and `StopVM.orderIds: string[]` beside the joined `order` string (same joined/split pattern as `AddressVM`) so stops render one link per order. Also `userDefinedData` on the top-level VM.
- **The stops arc — three passes, two of them mine to fix.** User reported "all orders repeated on all stops". (1) I fixed delivery splitting, then **over-corrected on a wrong theory** — gating order count by mode so multi-order shipments were TL-only, which cost **30% of seeded orders (4433 → 3123)** to eliminate a shape that is legitimate. Reverted same day when the user re-read the stop list and pointed out the obvious: an order appears twice by design, once per side. (2) The two GENUINE defects the report surfaced: `deliveryStopCount` was hardcoded to 1 (deliveries could never split) and `shipToLoc` was hardcoded to the shipment destination, so an order contradicted its own delivery stop. (3) The user asked whether any doc said deliveries couldn't split — **searching found the opposite**: Jana, Feb 17 grooming, twice ("if it has multiple deliveries, it will show delivery one, delivery two"; "the delivery date will be the last delivery"). **Lesson: search the canon BEFORE changing the generator, not after two rounds of pushback.** That same quote exposed a further defect — every stop on a multi-stop shipment carried the SAME clock time; stops are now sequenced 3h apart with pickup 1 on the shipment's pickup instant and the LAST delivery on its delivery instant.
- **`/analyze` on a real Tracking payload** (page 43 of 18,971 records) → new domain canon: `tracking-payload.md` (12-row divergence table), `data/tracking-api-contract.md`, `decisions/decision-log.md` (**TR-01…TR-08**), `20-cross-cutting/api-integration/tracking-service-api.md`, `_moc.md` rewritten. Document **`0200994650` = 1 pickup + 3 deliveries on mode `TT/ISO`** — real production proof that mixed shapes are valid, that a pickup legitimately carries every order while each delivery carries one (invariant I3 confirmed), and that **gating stop-splitting on `mode === 'TL'` made a production shape unrepresentable** → **DEC-69**: only LTL is capped at 2 stops (`canMultiStop = mode !== 'LTL'`). Measured after regen: LTL **0/911** multi-stop, David's rule exact. Subagent independently corrected three of my briefing assumptions (`trackingMessage.stops[]` empty in all 86 entries; `relatedShipments` has two semantics; a second `TERMINAL`/`DESTINATION` stop vocabulary) — all verified.
- **Aggregation / Consolidation, settled (DEC-68).** Relayed from Jana as two sibling patterns; I wrongly implemented it as an exhaustive constraint (reverted — see the memory below), then hypothesised a lane-based reading. **Rovo settled it against LINX docs: Consolidation is the UMBRELLA** ("grouping of orders on a shipment") with subtypes **Aggregation** (same O/D pair) and **Multistop** (multiple pickup **and/or** delivery points). My hypothesis was half right — Aggregation is same-O/D — but Consolidation is the parent, not the contrast. Every shape we seed is classifiable; **no constraint needed or wanted**. Open: obtain the consolidation XML's `SubShipmentList` / `PickupSequence` / `DropoffSequence` — an authoritative order↔stop binding we have never seen.
- **Timezones → IANA (TR-04).** `CITY_TIMEZONES` maps city → IANA zone; new `tzAbbrev(zone, date)` derives the abbreviation per-instant via `Intl`. Fixes a real DST bug (we emitted `CST` in July) and lets `America/Phoenix` correctly stay MST year-round. Stops carry an IANA `timeZone` field mirroring the real contract. **A split survives deliberately:** stops store IANA, but the ORDER wire's paired field is `*TimeZoneCode` and wants the abbreviation — `PickupDeliverySection` now derives it from the party city AND that field's own date.
- **The seed bug this nearly caused.** `seed.mjs parseDisplayDate` matched ` CST` **only**, with a hardcoded `-06:00`. Measured before running the reseed: **2,823 of 4,400 shipment timestamps would have been inserted as NULL** — silently. Fixed with an abbreviation→offset lookup (unknown zone → `null`, never a wrong instant) + 5 assertions. Now 4,400/4,400 parse.
- **Reseed executed and verified** (19m21s): 10,000 shipments / 24,057 orders / 23,235 stops / 45,146 tenders / 84,769 events / 13 users. Live probes: **0 NULL timestamps · 23,235/23,235 stops carrying IANA · 14,384 daylight vs 8,851 standard · 10,000/10,000 shipments with UDF data · 0 LTL over 2 stops · 1,626 shipments with 2+ deliveries.** Sample `25000693`: 2 pickups + 2 deliveries, orders distributed, times sequenced, Detroit correctly `America/New_York`/EDT.
- **Modal browser-verified** (puppeteer, `--no-save`, against the reseeded DB): dialog **920px on both tabs**, content 872 + 32px bottom pad, tab row flex/space-between with a **0px gap** to the strip, QCP in the row at 32px and surviving the tab switch, no footer, strip hidden on the UDF tab, GroupTable rendering real seeded UDFs, **0 console errors**.
- **The resolve-test treadmill, ended.** Every generator change re-rolls the faker stream, which invalidated `resolve.test.jsx`'s hardcoded order in S100, S101, and **twice more this session**. The test now DERIVES its order from `orders.json` at load (errorCount 5 / Shipment Failed / Ready, seeded errors spanning `general.*` + `consignor.postal`) and throws a clear message if nothing qualifies. No hardcoded order number remains.
- **Ops lessons.** (1) The first reseed attempt was **killed mid-run after `migrate --reset` had already dropped the old data**, leaving Neon at 3,800 shipments / 0 orders and live mode broken — a reseed is only atomic if it finishes; check DB state immediately when a long job dies. (2) A backgrounded 10-min job exceeded the 600s foreground cap and had to be re-run; the real seed takes ~19 min now that stops split. (3) Dev-server ports 5173–5178 were all held by stale servers — target the port explicitly (S101/S102 zombie pattern, third occurrence). (4) A concurrent session's uncommitted `GroupTable` work sat in the tree throughout; `wrap-commit.sh` stages everything, so this session committed an explicit file list instead.
- **Memory saved:** `feedback_relayed_concepts_are_not_instructions` — stakeholder concepts the user relays are a request to CONFIRM against our model, not a ticket; and a descriptive taxonomy is not an exhaustive constraint. The tell: writing "open question — is this exhaustive?" in the same message as shipping it.
...
- **Orders — Edit Order (ORD-12 / LINX-10248):** the blank-form report root-caused as `getDraft` **throwing** in live mode — the rejection killed the hydration chain before the `getOrderView` fallback ran, so every live Edit opened empty while mock (and therefore the suite) stayed green. Returns `null` now + regression test. Then: "Edit Order" chrome (PageHeader, breadcrumb, and the navbar title via a new `title` on `CreateOrderModeContext` — the navbar string was hardcoded, which is why the header still read "Create New Order" after the first pass), Order Number + Customer locked as identity, and the **Confirm & Save Changes / Discard** dialog. That needed a real write path: **`PUT /order-service/v3/order`** (`updateOrder`, update-then-insert projection, identity columns never written, `last_edit_at` stamped). Also caught: Save / Save for Later in edit mode routed through `saveDraft` — 404 in live (no manual-order POST route) and a duplicate row in mock; both now write in place. **No reseed was needed for the data** — 702 Draft orders, 375 with full `manual_order` (53%), and lean rows still hydrate through `listRowToManualOrder`. Audit trail (LINX-13853–13866) deliberately NOT built (user call).
- **Shipments — ShipmentDetailsModal rebuilt** to Figma 1348:16364: four full-width stacked sections × 4-column `TitleSubtitle` grid, `--` for empties, weekday suffix on stop times, `Planning Date Type` de-duplicated (mock has it only under Order), Hazardous simplified. Error variant (1357:17184) wired to the already-plumbed `detailsError`. **ModalMedium cap 780 → 920** (user).
- **Tender — one QuoteModal, three modes** (Add 1175:39228 / Rate Details 1408:21725 / Edit 1408:23260), 412 → 246 lines: the hand-rolled overlay + raw `<select>`/`<input>` replaced by ModalMedium + FormField + ComboBox + MeasureField. Rate Details renders footerless with ONE blocked composed `MM/DD/YYYY HH:MM CST` field per party; timezone is static UTC-offset text (system-determined, not a control); pricing fields normalize to 2dp on blur (`decimals` opt-in on MeasureField — off by default so a 22,416 lb weight never becomes `22416.00`); charge rows re-tracked (code 84px + `min-width:0` — the desktop 150px field floor was overflowing the track), trash 20px. SCAC now uses the shared paged carrier lookup, deleting a stale 15-row local copy.
- **Tender data wired to the DB:** the `tenders` table was seeded 1:1 from `shippingOptionList` but **never read** — the detail endpoint returned the frozen `shipments.detail` blob, so every Add/Edit Quote lived in component state until you switched shipments. `sellShipmentDetail` now merges tenders over the blob (falls back when empty), **`PUT /shipment-service/v1/sell-shipment-out/:id/tender`** persists one option by rank (update-then-insert — no unique index to migrate), and add/edit/status actions all write, including both rows of the Decline/Cancel cascade.
- **The field-height arc (38 → 36).** First attempt patched `.form-field__control` padding — arithmetic tuned to one variant, which left every leading/trailing-FieldSelect field at 38 because `.field-select` carried its own 8px. User pushback ×2 ("edit the components, not the usage") → structural fix: `.form-field__input` owns `height: var(--field-height)` (border-box), children carry no vertical padding, so EVERY variant is 36 by construction. Also landed: **`format`** on FormField (`text|integer|decimal|phone` — strips disallowed characters before `onChange`, which fixed the amount field wedging permanently on `Number('abc') → NaN`), the **`search-field*` → `combo-box*`** CSS rename (legacy from the S100 component rename), ComboBox restructured to FormField's column+4px-gap model (was margin-driven and drifted), chevron fields → 10px right inset, icon gap 8 → 4px, TimePicker `min-width` 130px, and a Title Case placeholder sweep across 14 files. The rename orphaned `.co-party .search-field` in create-order.css — caught only when the user reported Add Location collapsing.
- **Timezone ruling → a real seed bug.** User: timezone is the SHIPMENT's context, never the carrier's (which is why Orders carries no timezone spec). The generator hardcoded `pickupTZ/deliveryTZ: 'CST'` and appended a literal ` CST` in `formatDateTime`, so a Denver pickup read `(UTC-06:00)`. `CITY_TIMEZONES`/`deriveTimezone` moved into `data-pools.mjs` (single source, re-exported by master-data), zones derived per origin/destination and per stop city. Orders Pickup & Delivery lost its Time Zone control entirely — already auto-derived from the party city, so the value still reaches the wire. Live distribution after reseed: CST 25271 / EST 9138 / MST 5901 / PST 4559.
- **Batch lifecycle + Figma-first.** Five components (ModalMedium, FormField, FieldSelect, ComboBox, TimePicker) demoted → **approved** → **ported** → **released 0.10.0**. Version deliberately NOT 0.9.2: on a `0.x` package `^0.9.1` allows `<0.10.0`, so only the minor bump forces a conscious upgrade past the breaking ComboBox class rename (CHANGELOG carries a 12-row migration table). **Figma synced BEFORE release** — `Field/height` = 36 in `4. Sizing` (VariableID:4801:14; named `Field/*` so the audit's mechanical mapping yields `--field-height`), and ModalMedium's master gained `maxWidth: 920` — it had `minWidth 350` and **no max at all**, so the 780 cap had only ever existed in CSS. Snapshot 127 vars, `tokens:audit` aligned, user published.
- **Phase 3b, measured for the first time this session.** Puppeteer introduced (`--no-save`) after the user reported Angular fields still at 38. Measurement proved the code was correct and the **`ng serve` was 11 hours stale** (S101's zombie pattern again) — `--field-height` resolved to 36 but `.form-field__control` still served 8px padding. Restart + cache clear → both stacks measure identically (11 × 36px, ComboBox bar 36, FieldSelect 34 = 36 − borders). The port gate caught 4 real Δ≠0 gaps (icon gap, chevron inset, ComboBox select inset, TimePicker placeholder) and the release gate caught **ModalMedium's twin still at 780** — fixed and re-measured at 920 both stacks.
- **Ops lessons:** a library `dist` rebuild does NOT reach a running `ng serve` — restart after every `ng build odyssey-ui`. Measuring the wrong element lies: `.form-field__input` reports `padding: 0` because the 8px lives on `.form-field__control`.

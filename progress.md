...
1. **Error validation features (user-stated).** Also unblock + live-QA the confirmation cases: the create 404 = live-mode Submit gap (no `/order-service/v3/manual-order` on our API — S94 carry), so either build the create endpoint slice or QA in mock mode.
2. **Resume the parked /analyze intake** (PO feedback + Functional Req tracker — classification approved, synthesis subagent never ran). Feeds the feedback-fix worklist; note many PO items clear with a fresh deploy (permission-gated).
3. **Product Info open data questions (fold in when answered):** NMFC value-catalog consumer field (`PRODUCT_CLASSES`), product dropdown row anatomy (ID+description = Figma-first), special-services real codes (dev lookup capture — swap the invented mnemonics); confirmation-card gaps: net Product Weight + Tare Weight not captured (owed to Ramesh).
4. **Orders/Create remainder:** "Add More Details" row expansion (LINX-8131), dropdown footer "Don't see your address?" question (Efrain), LINX-8042 address cascade, appointment propagation (13869), Edit-flow live gap.
5. **Batch re-approval + Angular port batch (growing):** ModalMedium, Paginator, DataTable, **ComboBox (36px + panelError + Enter-commit + error placeholder + S97 portal/height-cap/typable/chevron-toggle)**, **MultiSelect (two-column dropdown + 100% width + emptyTableMessage)**, **FieldSearchResults (columnHeaders/rowHeight)**, **MatchSimpleRow (plain-list rule + twoColumn)**, ShipmentsBar (Figma sync owed), DatePicker/TimePicker/CalendarPicker, Accordion, S90 error-state components, `--spacing-9` token → Angular tokens file. Figma syncs owed: MultiSelect 4536:5333, MatchSimpleRow 3169:2821, ComboBox typable note (Efrain). Angular DSM flags staged locally (no push).
6. **End-of-Orders reseed** (permission-gated, one motion with deploy): `vault/10-domains/orders/db-update-ledger.md` shopping list.
7. **DB slices 4–7** (lookup endpoints feed the create-order comboboxes). rtk trial evaluation (`rtk gain`). PR #12 watch (blocked on `gh auth login`).
8. Carried: S87 punch-list remainder, 3 real API gaps, Home 1x-title wrap, MiniMultiselect (Efrain), ActionMenu cell-click flicker, 9.3MB chunk code-split, `.worktrees` cleanup, DesignSystemMap legacy refs. Orders S94 remainder: Export deploy-verify, Filter decision, live Submit/Cancel, Resolve→OIF (LINX-11137), location-sort semantics.

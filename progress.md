...
1. **Special Services section (user-stated):** build/conform §Special Services — TypeaheadSelect → ComboBox migration is part of this (real special-services catalog codes already in master-data).
2. **Product Information last layout fixes (user-stated):** final polish pass on the grid layout. Open data questions from S96 to fold in when answered: which field consumes the NMFC value catalog (`PRODUCT_CLASSES`), product dropdown row anatomy (ID+description would be Figma-first).
3. **Orders/Create remainder:** "Add More Details" row expansion (LINX-8131 — hazmat severity vocab + packing groups now captured), dropdown footer "Don't see your address? + Add manually" question (Efrain), LINX-8042 address cascade, appointment propagation (13869), Edit-flow live gap.

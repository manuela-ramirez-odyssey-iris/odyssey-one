...
1. **OPENER (user-stated): Shipments additions + kick off the OVERFLOW BOARD — a NEW domain.** No canon exists yet — expect a domain-intake arc (sources → vault `10-domains/overflow-board/` → backlog OVB- items) before any build. Clarify with the user what "overflow board" is and where its sources live.
2. **PR #13 merge watch** (Build Check green, awaits Cognizant review; publish theirs). PR #12 confirmed MERGED this session.
3. **Efrain flags:** FormField Figma Disabled text DSN/900 vs code `--text-tertiary`; the S100 batch (Layout axis approval, uppercase text-style, selected-state convention).
4. **Orders remainder:** live `getDraft` gap, "Add More Details" (LINX-8131), LINX-8042 address cascade, appointment propagation (13869), net/tare weight owed to Ramesh, LINX-11137 Ready-lifecycle question, freight-terms sort wrinkle, demo-parity known-red trio. Optional polish: orderId-based newest-first sort (free-text numbers currently stack first on desc).
5. **Resume the parked S98 /analyze intake** (PO feedback + Functional Req tracker — synthesis never ran; fresh deploy clears many PO items).
6. **DB slices 4–7** (lookup endpoints feed create-order comboboxes). rtk trial evaluation (`rtk gain`).
7. Carried: S87 punch-list remainder, 3 real API gaps, Home 1x-title wrap, MiniMultiselect, ActionMenu cell-click flicker, 9.3MB chunk code-split, `.worktrees` cleanup, DesignSystemMap legacy refs, S94 Export deploy-verify + Filter decision + location-sort semantics.

---

## Session 99 — July 28–29, 2026

**ERROR VALIDATION SHIPPED END-TO-END (ORD-10 / LINX-11137) — a full-arc session: FormField/ComboBox validated+error+focused states normalized Figma-first, then the OIF resolution behavior built via subagent-driven plan execution (7 tasks, two-stage reviews each), then two live browser-QA rounds with the user that root-caused a scroll-anchoring flicker and reworked the dock model.** Tests: app **535 → 560/560**, build clean. No deploys, no DB writes, no Angular pushes. GATE B stamped on ALL 14 staged components — batch approval + port kickoff is next session's opener.

- **Normalize cycle (user-directed opener):** FormField gained `State=Validated` + `Focused Validated` in Figma (new semantic **`Border/success`** → Caribbean Green/200, stroke-scoped; validated = success border + trailing check @DSN/500 + green "Validated" line; focused = 2px CG/600 mirroring Focused Error) and a `validated` prop in code; ComboBox gained the whole `State` axis (Default | Focused | Error | Focused Error | Validated | Focused Validated × Search | Select — 12 variants), `validated`/`required` props, and its error-parity gaps closed (message id + aria-describedby, error renders in ALL modes, label `*`). Token snapshot refresh owed (`Border/success` postdates the 2026-06-22 dump).
- **The resolve feature (spec → plan → subagent build):** LINX-11137 AC (story screenshot) + 6 Efrain mocks intake'd; approach = **CreateOrderForm `?resolve=` mode**, error source = **deterministic derive+seed** (`resolve/validationErrors.js`, the future-OIF seam). Full detail in **ORD-10**. Review loop caught real bugs each round: zod-suppression on resolved fields (Save could've enabled on bad data), seeded errors landing on unrendered fields (`manualMode` regenerated false — hydration now force-reveals), keyboard hole in the CSS lock (focus-bounce guard), Alert docked-stepper clamp (→ Alert demoted + re-approved same session), live-mode unhandled rejection (one-line catch). `test.env` now pins `VITE_API_MODE=mock` for the whole suite (a `.env.local=live` leak had one resolve test failing silently).
- **The dock saga (two user QA rounds, real Chrome):** round 1 — docking flickered + pinned scroll to top: **Chrome scroll anchoring** reacting to the dock morph's intermediate layout; fix = `overflow-anchor:none` for the session + height compensation. Round 2 (user model, better): expanded alert is plain in-flow (scrolls away naturally), docks only once its **last list item** passes the navbar, **collapsing in the same motion** (return trip shows the collapsed bar — no spacer hole), one pre-paint scrollTop compensation; error nav = a **session** that stays docked until a deliberate return to top, with the scroll floored so the bar always pins flush under the navbar; docked bar full main-width with contents aligned to the 1080px column; slide-in/fade transitions. Component graduations (user call): Alert now owns the dock slide-in, the **all-resolved success surface + CircleCheck**, and `role="status"`.
- **Also:** orders table row-click REMOVED (kebab View is the single entry — user call); ledger rows 7–8 (validation errorCount weighted-low at reseed, never uniform-high; +1000 records next session); `deriveValidationErrors` hardened per review (honest count clamp, non-mutating `applyErrors`, derived section).
- **Parked / carried:** refresh of a `?resolve=` URL loses the row's errorCount (falls back 3 — closes when a real errors payload lands, ledger row 7); product-grid resolve errors deferred; ComboBox option rows lack listbox semantics (pre-existing DSM a11y gap); undocked ComboBox full-bleed scrollbar caveat.
- **Post-wrap Q&A (user):** live-mode Purge "does nothing" explained — the user's local dev runs `VITE_API_MODE=live` and our API has no status-write path, so the live guard throws and the catch swallows it; **ledger row 9** added (status-update endpoint closes live Purge/Resolve/Submit/Cancel in the reseed+deploy motion). Resolve-button gating confirmed as Jira-required (LINX-11659: enabled only on `draftOrderStatus === 'Ready'`; Complete/Purge = past user action) — **flagged for Ramesh:** the AC's own `Ready` definition reads post-resolution, sits oddly with LINX-11659 making it the resolvable state; confirm the lifecycle when LINX-11137 exits Analysis.

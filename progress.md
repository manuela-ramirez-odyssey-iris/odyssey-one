...
1. **Finish the S87 automation punch-list** (Tier 1+2 done S88 — see the S88 entry). Remaining: **Tier 3** — `dsm-flags --demote` (automate the one manual lifecycle step: the modification-reset back to NORMALIZING, both DSMs), decide the `PORTING` badge (wire into `dsm-flags` vs delete the unreachable `DesignSystem.jsx:127` render), turn `port-readiness`'s dep-order print into an `existsSync` check (catches the PageHeader→ButtonToggle trap), auto-stamp `figma-link.md` `last_synced`; **Tier 4 hygiene** — gitignore `.connect-publish.last`, fix `verify-all` fragile stat regexes, widen `demoPrefix`, guard `scaffold-port insertAppModule` anchor.

...
1. **SEEDING (user-stated opener):** subagent-driven planning + execution of the regen/reseed incorporating ALL ledger rows 1–8 (`vault/10-domains/orders/db-update-ledger.md`) — real equipment/freight-term/ship-direction codes, handling-unit codes, 18-digit product IDs, **validation errorCount weighted-low (never uniform-high)**, **+1000 more order records**. Permission-gated one motion with deploy.
2. **AT THE SAME TIME — approve the batch → Angular port kickoff:** all 14 staged components sit at APPROVED (GATE B, stamped S99); the user's batch-approval command triggers `/port-to-angular` for the whole batch. Port list: ModalMedium, Paginator, DataTable, ComboBox (S94 rename + S97 portal/typable/chevron + S99 states/required/rootClass/aria), MultiSelect, FieldSearchResults, MatchSimpleRow, ShipmentsBar, DatePicker/TimePicker/CalendarPicker, Accordion, Alert (S99 stepper clamp + success surface + role=status), StepIndicator, FormField (S99 validated), `--spacing-9` + `--border-success` tokens → Angular tokens file. Figma syncs owed: MultiSelect 4536:5333, MatchSimpleRow 3169:2821, ComboBox typable note + token snapshot refresh (Border/success), Efrain notes.
3. **Resume the parked /analyze intake** (PO feedback + Functional Req tracker — classification approved, synthesis subagent never ran). Many PO items clear with a fresh deploy (rides with the reseed motion).
4. **Orders remainder:** create 404 live-mode Submit gap (no `/order-service/v3/manual-order`); "Add More Details" (LINX-8131); LINX-8042 address cascade; appointment propagation (13869); Edit-flow live gap; Product Info data questions (NMFC value-catalog consumer, product dropdown row anatomy, special-services real codes, net/tare weight owed to Ramesh); resolve-mode carries (refresh loses errorCount — closes with ledger row 7; product-grid errors; ComboBox listbox semantics).
5. **DB slices 4–7** (lookup endpoints feed the create-order comboboxes). rtk trial evaluation (`rtk gain`). PR #12 watch (blocked on `gh auth login`).
6. Carried: S87 punch-list remainder, 3 real API gaps, Home 1x-title wrap, MiniMultiselect (Efrain), ActionMenu cell-click flicker, 9.3MB chunk code-split, `.worktrees` cleanup, DesignSystemMap legacy refs, S94 Export deploy-verify + Filter decision + location-sort semantics.

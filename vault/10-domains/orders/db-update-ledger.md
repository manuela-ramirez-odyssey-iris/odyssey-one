...
| R2-10 | **OIF Level 1 columns: `interface_error_count` / `interface_error_class`** (Orders) | The generator has seeded both on Validation-Errors rows since S145 (LINX-16049 resolution cases, LINX-16028 flag+count), but `orders` had no columns for them: live mode returned `interfaceErrorCount` null on every row, Step 1 of the resolution flow was unreachable, and the grid's "Errors Count" (structural + master data, Ramesh 2026-09-10) silently degraded to the master-data half. Tracked as [[open-questions#Q-OIF-4]]. **Code shipped S145 (2026-09-10)**: `packages/db/migrations/010_orders_interface_errors.sql` (two nullable columns), `tools/seed.mjs` writes them, and `api/_lib/orders.mjs` projects them + routes the Errors Count comparator, bar chip and sort through one `ERROR_COUNT_TOTAL` = `(error_count + coalesce(interface_error_count, 0))` — the SQL twin of `totalErrorCount`. **Still owes the run**: `migrate.mjs` (no `--reset`) then `seed.mjs --reseed`, permission-gated as always; until then live behaves exactly as before. | 2026-09-10 |

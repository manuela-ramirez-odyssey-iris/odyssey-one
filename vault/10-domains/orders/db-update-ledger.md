...
| R2-10 | **OIF Level 1 columns: `interface_error_count` / `interface_error_class`** (Orders) | The generator has seeded both on Validation-Errors rows since S145 (LINX-16049 resolution cases, LINX-16028 flag+count), but `orders` had no columns for them: live mode returned `interfaceErrorCount` null on every row, Step 1 of the resolution flow was unreachable, and the grid's "Errors Count" (structural + master data, Ramesh 2026-09-10) silently degraded to the master-data half. Tracked as [[open-questions#Q-OIF-4]]. **Code shipped S145 (2026-09-10)**: `packages/db/migrations/010_orders_interface_errors.sql` (two nullable columns), `tools/seed.mjs` writes them, and `api/_lib/orders.mjs` projects them + routes the Errors Count comparator, bar chip and sort through one `ERROR_COUNT_TOTAL` = `(error_count + coalesce(interface_error_count, 0))` — the SQL twin of `totalErrorCount`. **RUN 2026-09-14 (S147, user go: "yes you can do a reseed")**: `migrate.mjs` then `seed.mjs --reseed` — 23,838 orders / 1,430 VE rows; verified through the local API: 17 of the first 40 VE rows carry `interfaceErrorCount` > 0 (e.g. `0000000091140` structural 1, `0000000091163` mixed 2). Live Step 1 is now reachable. The deployed function serves the columns as soon as it is next deployed (nullable, additive). | 2026-09-10 → run 2026-09-14 |

## 2B — Code only (NO reseed needed)

| # | Item | Detail | Logged |
|---|---|---|---|
| R2-5 | **~~Order creation doesn't persist~~ — SHIPPED** | ✅ **Shipped 2026-08-02** in `ea12d96` (`POST /order-service/v3/manual-order`), never marked here until S113. Drafts share the same path via `orderStatus.orderStatusCode: 'DRAFT'`. **S113 closed the remaining gap**: Save for Later required an Order Number that create leaves blank, and re-saving 409'd because the live branch always re-POSTed — see **ORD-15**. Original ask: Create Order must write to the DB and then show the defined summary/confirmation (LINX-9002). | 2026-08-01 |
| R2-6 | **Appointment flag invisible in Edit flow** | **Definition found in vault** — David Johns, Apr 9 2026 (`grooming/feedback/0409-jana-david.vtt` 15:12–15:49): a pickup/delivery date normally expresses an early/late **window** (*"that's a pickup between noon and 8:00 PM"*); checking the Appointment box **collapses the window to a firm slot** (*"then that noon becomes the appointment"*) and **must be transmitted to the carrier** (*"the carrier knows … you have an appointment to pick it up at"*). Related: DEC-34 (time is only meaningful for appointments), DEC-36 (in the **Shipments** Order tab the checkbox is **display-only**). Open: in **Orders** it should presumably be editable — confirm with Ramesh. ⚠️ **REOPENED 2026-08-07 (S113)** — user reports the flag still does not survive an edit round trip on the deployed app. The S110 `dirtyFields` fix is confirmed present and correct, and a form-level test proves every hop (including the uncheck direction); the DB shows all 4 app-written orders stored `N`. **Cause unproven — needs a browser repro. Deliberately NOT fixed on hypothesis.** See **ORD-17**. | 2026-08-01 |
| R2-7 | **Appointment & Hazardous flags invisible in View Order** | Hazardous is already specced: per LINX-8121 the line-level checkbox auto-checks when a UN Number is entered, and *"if at least 1 product in the order is hazardous, the entire order is considered as hazardous"* — so View Order needs the derived ORDER-level flag plus the per-line flag. Appointment per R2-6. ✅ **Hazardous shipped** (`653e0a0`) — though S113 found its column was inserted at a position matching no canon and moved it to the LINX-8121 order (**ORD-13**). ✅ **Appointment display FIXED in S113**: `mapSellShipmentOutToDetail` emitted a raw boolean into a JSX child, so a **checked** appointment rendered blank while unchecked rendered `--`; now `'Yes'`/`'No'`. The existing test had encoded the bug as expected behaviour. Edit-side persistence remains open per R2-6 / **ORD-17**. | 2026-08-01 |
| R2-8 | **~~Product Information columns not inline with LINX-13893~~ — SHIPPED** | ✅ **Create** conformed in S95 (**ORD-08**); ✅ **View Order / confirmation / Shipments Orders tab** conformed in S113 (**ORD-13**) — the read-only card now imports `columnsForEquipment` rather than duplicating the matrix, and the whitelist mapper that was discarding Handling Unit / dims / Harmonized / Declared Value / STCC was removed. STCC-vs-8131 conflict and `TLF`'s missing case still owed to Ramesh. Original detail: the columns are **not a fixed set** — LINX-13893 ("Lauren's feedback") is an **equipment applicability matrix**: **Case 1** LTL/LTR/LTH → +Product Class, +Handling Unit Name/Description/Count, +L/W/H; **Case 2** TL/TLR/TLH/TT → same **minus Product Class**; **Case 3** LCL/FCL (Ocean) → +Harmonized Code, +Declared Value & Currency, +Manufacturing Country Code; **Case 4** RR (Rail) → +STCC (non-mandatory, TMS-validated, error *"Incorrect STCC Code. Please check the value & re-enter"*). Full text: `research/jira-create-order-sections-2026-07-26.md` §3. **Known unreconciled conflict:** STCC is struck from the base grid (8121/8131) but live for Rail (13893 Case 4) — flag to Ramesh. `TLF` exists in the legacy catalog but appears in **no** matrix case. | 2026-08-01 |

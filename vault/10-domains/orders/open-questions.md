...
- [ ] **Q-OIF-1 — does a message that fails `sourceSystem` / `modifyTimestamp` / `relySourceId` reach the resolution queue at all?** These are LINX-16049 rules 8, 9 and 11 — message-control (provenance) fields the planner cannot author. If such a message IS queued, what is the planner's exit: is there a re-send, a purge, a support hand-off? Ramesh had no answer (technical). The build shows them read-only with the backend message, no action, and **blocks `Validate and continue`** while one exists — a dead end by design, pending this answer. Owner: **Venkat**.

- [ ] **Q-OIF-2 — the Step 1 save contract.** Ramesh ruled (2026-09-10) that Step 1 fixes are saved *right away*, not at the final Save. Which endpoint receives the picked values and the structural fixes, and **does it return the Level 2 error set in the same call**? LINX-11137 §E says validation is real-time and sequential ("immediately performs Order Data Error validation"), which implies one round trip. The build writes through the existing order-update path (mock overlay / local API `PATCH`) with `interfaceErrorCount: 0`, and derives the Level 2 errors client-side in `interfaceErrors.js` / `validationErrors.js`. Owner: **Venkat**.

- [ ] **Q-OIF-3 — is editing the CUSTOMER's data acceptable on an integrated order?** Cross-line conflicts only ever CHOOSE between values the customer actually sent (rule 5: City from line 1, Postal Code from line 3). Structural fixes do not — rule 2 has the planner type a new gross weight / package count / volume onto a line, and rule 4 has them supply a time zone the message omitted. That rewrites the customer's message rather than resolving an ambiguity in it. Ramesh's ruling #6 ("the planner fixes structural line faults") is what we built; **confirm at the regroom** that it is acceptable for an integrated order, and whether the original values must be retained for audit. Owner: **regroom (Ramesh + Dave)**.

- [ ] **Q-OIF-6 — a locked timeline step renders a GREY CHECKMARK.** `ResolveTimeline` maps a not-yet-reached step to `StepIndicator status="off"`, and `off` in Figma is a grey circle **with a check inside** (the atom was designed for a vertical form stepper, where "off" only ever means "not filled in yet" next to a live section). On a horizontal progress timeline the same mark reads as *"done, greyed out"* — the exact opposite of "locked, not reached". Needs a design decision: an empty circle, a step number, or a third `StepIndicator` status. It touches a **normalized atom** and therefore owes a Figma mirror before any code change, so it is not ours to settle. **Deferred to Efrain by the user, 2026-09-10.** Owner: **Efrain**.
...
- [ ] **Q-OIF-7 — in LIVE mode a resolved order stays on the Validation Errors tab.** `resolveOrder` can only write the lifecycle half (`Ready for Planning`): the status PATCH whitelist is `['Ready for Planning','Cancelled']`, and there is no endpoint to write the OIF `Complete` or to clear `draft_order_status` — which IS the Validation Errors population predicate. So a successful Save in live leaves the row where it was. `purgeOrder` has no live path at all and deliberately THROWS rather than writing `Cancelled` (a different business fact — a real lifecycle status the user can Restore from). Both are honest failures rather than wrong data, but neither is a shippable end state. Needs the OIF write contract (same conversation as [[#Q-OIF-2]]). Owner: Venkat.

- [x] **Q-OIF-4 — `interfaceErrorCount` / `interfaceErrorClass` were mock-only. RESOLVED 2026-09-14 (S147): migration 010 + reseed RUN on the user's go; live Step 1 reachable (ledger R2-10).** *(Original text kept below.)* Original gap: the generator seeded both on Validation-Errors rows, but Neon had no `interface_error_count` / `interface_error_class` column, so `mapOrderListRow` yielded `null` in live mode, every order opened at Step 2, and the grid's "Errors Count" (structural + master data, Ramesh 2026-09-10) silently degraded to the master-data half. **Shipped in code:** `packages/db/migrations/010_orders_interface_errors.sql` adds both columns (nullable); `apps/odyssey-one/tools/seed.mjs` writes them; `api/_lib/orders.mjs` projects them and routes the Errors Count comparator, bar chip and sort through a single `ERROR_COUNT_TOTAL` = `(error_count + coalesce(interface_error_count, 0))` — the SQL twin of `totalErrorCount`. **Not yet true of the live app:** the user must run the migration and reseed (permission-gated, see [[db-update-ledger#Round 2 — opened 2026-08-01 (S104)|ledger R2-10]]) and redeploy; until then live behaves exactly as before — Step 1 unreachable, Errors Count master-data-only. Distinct from [[#Q-OIF-7]], which stays open.

## Audit trail (added 2026-09-14, LINX-8091 / LINX-9128 — spec `2026-09-14-order-audit-trail-design.md`)

- [ ] **Q-AT-1 — one row per SAVE or one row per FIELD?** Both ACs (Notes §1): *"capture all changes, in one row … This row should contain the list of all the fields that changed"* (9128: per modified line item; the matrix's Released-from-Hold row says "field name(s)"). Ramesh on the 2026-09-10 call, correcting Laura twice: *"each field that changed should be a row"* — location + date in one save = two rows. The build follows the AC (stories outrank transcripts) and renders Field / Old / New as aligned stacks, so either answer is a seed change; but it is the data model and `/v3/audit-report`'s row shape decides it. Owner: **Ramesh** (with Venkata for what the API actually returns).

- [ ] **Q-AT-2 — `/v3/audit-report` response shape.** LINX-8457 (Closed) embeds the field table as an image ("Refer table") that the Jira export did not carry; the LLD link is `Order Service Phase-2 § /order-service/v3/audit-report` on Confluence TMS/3401056276. Needed to write the adapter: field list, how line-level rows carry the line id, whether Source is email or an id we must resolve, paging + sort params. Owner: **Venkata**.

- [ ] **Q-AT-3 — what is the category for "line item added"?** 9128's scope clarification lists *"Adding one/more line item(s)"* as a tracked line-level change; its category table has no value for it (closest: *Order Line Item Editing* with blank Old Value). The seed does not emit it until it has a name. Owner: **Ramesh**.

- [ ] **Q-AT-4 — placement + title deviate from the AC; inform Ramesh.** AC: *"visible in the particular order's 'View Order' page (LINX-10233), as a separate button / tab titled 'Audit Trail & Activity History'"*. Built: opened from the Orders grid ⋮ menu, own page, titled *Audit Trail* (user ruling 2026-09-14 — "no secondary button"). Steve O'Hara's Feb-17 approval comment (*"select an order and view the audit trail"*) supports row-level entry. Ask whether View Order must ALSO link to it so the AC's sentence stays literally true, and whether the title change is acceptable. Owner: **Ramesh** (regroom).

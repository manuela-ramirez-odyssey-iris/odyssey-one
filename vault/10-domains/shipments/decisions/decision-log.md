...
### DEC-110: Reason Description leaves the column grid and becomes a full-width row
- **Previous:** `reasonDescription` was the FIRST column of the Dropped Carrier detail table at a 360px width — the widest column in the table, ahead of thirteen short ones.
- **Decision:** it renders as a labelled full-width row **below** the detail values (Start Date → Comment), spanning every detail column and wrapping. GroupTable gained a generic `group.detailNote` slot for it; the columns keep their horizontal real estate.
- **Source:** user, 2026-08-17 — *"we can have the reason separate because might get too wide … like if reason description was pushed below. This way we can have the horizontal realstate released."*
- **Why it is a library slot and not a one-off:** the pattern is "one long free-text field among short ones", which is not specific to dropped carriers. The consumer supplies the content; GroupTable owns the row.
- **The API is `{ label, value }`, not a raw node** (user, 2026-08-17 — *"so cognizant dont have to hack our component to achieve the same"*). The first cut worked only if the consumer hand-wrote `<span className="odyssey-group-table__detail-note-label">` — a component whose only route to a feature is its own internals is one every downstream team ends up reaching inside. The pair is now rendered AND styled by the component; a bare node stays accepted for what a label/value pair can't express. Carries to the Angular twin as the same prop shape.
- **Unchanged:** the SHORT `reason` label stays a column on the always-visible carrier row. The dash rule holds — the note renders `--` rather than disappearing, so "no reason given" can't be confused with "routing returned none" ([[#DEC-108|DEC-108]]).

### DEC-111: No info icon on the Dropped Carrier section header
- **Previous:** `SubAccordion` renders its `Info` glyph by default; the section inherited it.
- **Decision:** `showIcon={false}`. **Source:** user, 2026-08-17 (*"we dont need the info icon"*). No component change — the Figma master already carries the `Show Icon` boolean.

### DEC-112: The tender table renders on the canonical Cell contract
- **Previous:** hand-rolled inline `thStyle`/`tdStyle` — 12px UPPERCASE letter-spaced tertiary headers on a gray fill, 10/14 padding, `--bg-tertiary` hairlines. It predated the Cell contract and matched nothing else in the app.
- **Decision:** the split table now uses `.odyssey-table` + `.text-label-sm-*` — white 48px cells, 14/16 padding, `--border-subtle` hairlines, **semibold `--text-primary` sentence-case heads**, regular data cells, `--title` emphasis columns. The inner border/radius went with it: `.tender-pane__table-card` is already the surface, and the mock shows one frame, not two.
- **Source:** Figma **Tender Table `1596:21526`** (Shipments — Odyssey One), handed over by the user as the guide. Its variables resolve to `label/sm semibold` + `Text/primary` + `White` + `Border/subtle` — i.e. the Cell master our CSS contract was normalized from, so this is adoption, not new styling.
- **Side effect, on purpose:** the row tint moved from the `<tr>` to the `<td>`, because the Cell contract paints every cell white and a row-level background would be covered. Cross-table hover sync (left half ↔ right half) still needs JS — CSS `:hover` cannot reach a sibling table's row.

### DEC-113: Column arrangement returns to the tender table — with a panel of its own
- **Previous:** the control was **removed** on 2026-08-10 ([[#DEC-71|S114 round]]) because it called ShipmentTable's `onToggleColumnPanel` and opened the shipments-LIST panel. A control that opens the wrong table's panel is worse than none.
- **Decision:** restored in the pinned action lane's header exactly where the mock puts it, wired to its own `ColumnPanel` instance — the same generalized panel Product Information reuses (`allColumns` / `presets` / `defaultPresetId` props). Arrangement is **per sub-tab**: the sub-tabs ARE column groups, so the universe is the active tab's columns and switching tabs resets it. Pane-lifespan state, no persistence — matching the Product Information call site.
- **Source:** Figma `1596:21526` + user, 2026-08-17 (*"notice we have column arrangement functionality for it too"*). The 2026-08-10 removal note called itself reversible *"once a real routing-column panel is specced"*; this is that.
- **Portaled to `<body>` deliberately:** the expanded ShipmentsBar carries a `clip-path`, which makes it the containing block for `position: fixed` descendants **and clips them** — an in-place dock would be cut to the bar's box, and to nothing at all in the partial stage. Body level also lands it outside `<main>`, which BottomBar's outside-click rule already exempts, so opening the panel cannot collapse the bar.
- **Header geometry (2026-08-17, second pass):** the tender header is a **fixed two-line box (68px) in both halves**. Route Rank and Tender Status read better stacked, and the moment one header wraps the other half has to match it — two separate tables cannot sync that in CSS, so the height is stated (two 20px label lines + the contract's 14px pads) rather than left to whichever label happens to wrap. Tender Status's header width is 96px: at 78 the wrap worked but the word *"Tender"* itself ellipsized to *"Ten…"*. The pinned action lane is 68px wide (GroupTable's floor) because at 50px its content box was 18px and clipped the 38px arrange Button, and the collapse seam is a 36px white lane with a 20px glyph per the mock's Right Table Expander (`1596:21583`).
- **Found on the way:** `.odyssey-table__cell--title` never applied its colour. `.odyssey-table td` (0,1,1) out-specifies a bare class (0,1,0), so **every** Title cell in the app — ShipmentTable, Orders, the DSM demo — rendered tertiary while the contract, the Cell master and the code all said primary. Fixed at the source (`td.` prefix); the change is visible in those tables too.
...
- **Amended later the same day (final):** the tab signals became floating corner badges (SPB-82). Then the dropped-carrier signal was **removed entirely** — tab badge, section-title colour and ListX all gone — on the reasoning that dropped carriers are informational (canon §"What decision the screen supports": nothing about them demands action, and routing drops some on nearly every shipment, so a badge would light everywhere and dilute the one that matters). The **order-change badge stays**: a pending order change locks the Tender tab (S137), so it is a genuine blocker and now the tab's only alert. It renders **inline after the tab label** as the shared **icon-only purple `Badge`** carrying lucide **`OctagonAlert`** — a stop sign: halted until reviewed. Not TriangleAlert, which the app already spends on failures and warnings. The Review Order Change button carries **no icon** — its LABEL is purple instead (user, 2026-09-07, after three glyphs in a day). Four shapes were tried the same day before this one: a bare inline glyph, a floating corner circle overhanging the bar, a count badge, and a white tile with a coloured glyph.
...
### DEC-132: A consolidated order change reviews on the Stops tab; the Direct review route is Direct-only
- **Previous:** every `order-change` row — 218 seeded, 131 of them multi-order — opened `/shipments/order-change/:id`, the Direct review (single tendered carrier, cost radios, three tender actions). LINX-14509's own business rule says that flow *"applies only to Direct Shipments"*, and the seed diverts multi-order shipments into the category exactly like single-order ones.
- **Decision (S142, 2026-09-08):** the row action and the Tender-tab *Review Order Change* button branch on `shipmentType`: `Consolidation` selects the row and lands the bottom bar on the **Stops tab** (LINX-15435: *"The Stops tab shall be selected by default when accessed from an Order Change exception"*); `Direct` keeps the route. The consolidated review payload rides as `orderChange.consolidation` in the same `detail` jsonb (DEC-106 pattern), seeded only when `orders.length > 1` from an id-salted PRNG — zero faker draws, all 2,200 ids verified unchanged.
- **Source:** LINX-14509 + LINX-15435 ACs; seed measurement 2026-09-08.

### DEC-133: VD copy normalized to AC wording; three VD details ruled
- **Previous:** Laura's Stops Consolidated VD (`1910-31512`) reads *Edit Shipment Stop*, *New Consol Cost*, carries a seventh *Margin* strip cell, titles Stop 1's right column *Prior Tender List*, and the Planning Dates modal's first column is a `01/02` row index.
- **Decision (user, 2026-09-08):** AC wording wins — **Edit Shipment Stops**, **New Consolidated Cost**; **Margin dropped** (on the DTO but in no consolidation AC); all three right columns read **Affected Orders** (the P1 label was a paste error — the rows under it are order links, not carriers); Planning Dates' first column is the **Order number** (rows are orders, not product lines). Per [[feedback_efrain_copy_not_canonical]] applied to Laura's mocks.
- **Source:** user session S142; LINX-15435/15436.

### DEC-134: View Routing shows Dropped Carriers and badges cost differences on both lists
- **Previous:** VD `2108-14708` shows two tables (New above Prior) and badges a changed AP Cost on the Prior row; LINX-15438 lists *Prior Tender, New Tender, Dropped Carriers* plus the three costs.
- **Decision (S142):** a third table, **Dropped Carriers** (the new version's list — canon §7 "dropped carriers ride inside the review"), added under Prior; cost badges are **symmetric** (a SCAC whose AP cost differs between the two lists badges on both rows, as the VD and the Direct review's lists already do); the three costs are not repeated in the modal because they sit in the card head above it. SCAC-keyed matching collapses a carrier that appears twice at different ranks — accepted for a read-only modal, documented in code.
- **Source:** LINX-15438 AC over the VD's omission; VD for the symmetric badge.

### DEC-135: "View Routing" on the Stops tab, "Call Routing" in the Compare Screen — name follows purpose
- **Previous:** LINX-15669 names the sandbox action *Call Routing*; 15670, 15869, 15871 and the VD say *View Routing*; the Stops-tab button (15438) is also *View Routing*.
...
## Odyssey Shipment Identifier — DEC-144 … DEC-147

### DEC-144: the Odyssey Shipment Identifier is the lead column and the lead search attribute; Buy/Sell become internal
- **Previous:** DEC-66 — `buyShipment` = THE shipment ID (LINX-11591/12490/13023), buy-first default columns, Sell Shipment # second as the internal wire key.
- **Decision (S148):** a new attribute `odysseyShipmentIdentifier`, UI label **"Odyssey Shipment Identifier"**, takes first/far-left position in `ALL_COLUMNS`, all three default profiles, all three named views, and the head of the *Shipment Identifiers* search group. It is one ID **common to the shipment's buy and sell side** — in the real TMS schema buy and sell are separate rows sharing one `odyssey_shipment_id`; our row collapses the pair, so it is 1:1 with a row (and a multi-leg chain gets one per leg). Format: first character `O` for a single-order shipment, `C` for a multi-order (consolidated) one, then a system sequence starting at **50,000,000** so an O2 id cannot collide with a TMS-generated one. **Length is deliberately unbounded** — the sequence grows, so nothing pads, zero-fills or length-validates it. Buy and Sell Shipment # remain columns but are now internal-only: what goes to the customer and the carrier is the Odyssey Shipment Identifier plus the Planned Shipment ID (= sell). DEC-66 is amended, not reversed — buyShipment is still a real user-facing column, just no longer *the* identity.
- **Source:** Laurie (column/filter note + "Starting with 50M" update) and Dave Schultz (O/C format), both 2026-09-15, captured at `vault-sources/10-domains/shipments/sources/laurie-dave-odyssey-shipment-identifier-2026-09-15.md`. Field name is the Shipment Service LLD's own (`"odysseyShipmentIdentifier": "C813888"`, 15 occurrences); both prefixes appear in the Tracking payload (`O31905829`, `C836907`).

### DEC-145: it is a display + filter field, NOT the key — row keying, routes and detail links stay on sellShipment
- **Previous:** ambiguous. Laurie's note ("Shipment Identifier = a common ID for the Sell and Buy") could have been read as re-keying the grid.
- **Decision (S148):** Dave Schultz settles it — "`shipment_id` is the PK (no two rows have the same value) and you are already displaying that correctly in the UI. The `odyssey_shipment_id` is the one that is common across both buy and sell." So `ShipmentRowVM.id = sellShipment`, the `:sellShipment` route param, the `sell-shipment-out/{id}` contract path, the 2,200 per-shipment detail filenames and `nextShipmentId` chain linkage are all untouched. Re-keying would have broken every hardcoded id for a display change.
- **Source:** Dave Schultz, 2026-09-15.

### DEC-146: `match: 'both'`, not `digits` — the letter prefix is load-bearing in search
- **Previous:** both sibling identifiers (`buy-shipment`, `sell-shipment`) are `match: 'digits'`.
- **Decision (S148):** the new attribute is `match: 'both'`. A digits-only matcher would never suggest the chip for a query starting `C50…`, making the lead identifier unsearchable by its own printed form. For the same reason the Cognizant sheet types it **Text Input**, not Number Input. Registry priority 0 forced every other `SHIPMENTS_ATTRS` priority up by one (`registryParity.test.js` binds server priority to the client flattened index).
- **Source:** Dave Schultz — "Do NOT make assumptions on length (because the sequence grows)"; `search/shipments/progression.js`, `api/_lib/search-registry.mjs`.

### DEC-147: generated by a plain counter, never a faker draw — every existing id verified unchanged
- **Previous:** DEC-107 established the rule the hard way — an isolated Faker instance was needed to keep the main draw sequence intact, verified by a full-corpus diff.
- **Decision (S148):** the identifier is assigned from a module counter (`ODYSSEY_SHIPMENT_SEQ_BASE = 50_000_000`, reset in `resetGeneratorState()`), taken after `orderCount` is known so the prefix can be derived from it. Consuming no randomness means the draw sequence is untouched: all 2,200 buy/sell ids and every order number keep their current values, and no hardcoded id in the test suite dies. The prefix is **stored, not computed**, so it is frozen at creation by construction.
- **Source:** `tools/generate.mjs`; determinism verified against the pre-change dataset.

**Open (not ours to rule):**
- ~~**O→C flip.**~~ **CLOSED (Dave Schultz, 2026-09-15)** — "when you are creating a shipment, you know whether it only has one order or is a consolidation. So, at that moment, the correct prefix can be assigned. **A shipment cannot change from one type to another.**" Frozen at creation, as built (DEC-147 stores the string rather than computing it). **Tension raised and CLOSED the same day (Dave Schultz, 2026-09-15):** "you can't add orders to a direct shipment, you create a NEW Consolidation shipment using the Loads from other Shipments… once something is a 'C', it stays that way forever". Both halves already hold in our code — `AddOrdersModal` is rendered only by `EditStopsView`, reached only from the "Edit Shipment Stops" button, which renders only under `review = !!orderChange?.consolidation && !orderChange?.resolution` (`StopsTab.jsx:164,167`), so a Direct shipment has no entry point to gain orders; and a consolidation dropping back to one order keeps its `C` because the prefix is stored, never recomputed. The concern was raised from the `buildSaveStopsQuery` write without first checking the UI gate — the gate was already there.
- ~~**Sell Shipment # relabel.**~~ **CLOSED.** **RESOLVED (user, 2026-09-15): the Sell Shipment # column header stays as it is.** "Planned Shipment ID" is the outward-facing name for the sell shipment — what Odyssey sends the customer and the carrier — not the internal grid label. Laurie's own note puts Buy and Sell under "Internal Only", and our grid is the internal view. No rename in `ColumnPanel`, `ShipmentTable`, `progression.js` or `search-registry.mjs`; the contract name `plannedShipmentIdentifier` is mapped at the API boundary if and when the real contract lands.


### DEC-148: the Odyssey Shipment Identifier is the displayed name everywhere, but never the key
- **Previous:** DEC-144 put it in the grid, search and details modal; five other surfaces still printed `buyShipment` as the shipment's name — the default grid sort, both Order Change page headers, the bottom panel, and the search result rows in mock and live.
- **Decision (user, 2026-09-15):** every surface that names a shipment to a human names it by the Odyssey Shipment Identifier, falling back to `buyShipment` then `sellShipment`. The Order Change header reads *Shipment C50000003* in place of *Buy Shipment 12345*. Live search needed `REGISTRY.shipments.hydrate.columns` widened to fetch the column, or its rows would have rendered `undefined`. **Nothing about keying moved** (DEC-145): `data-shipment-key` stays `sellShipment`, and the search row `id` was verified to be a React list key only — `GlobalSearchResults.jsx` clicks through on `data-shipment-key`, so the displayed id cannot affect navigation. Four `navigate()` calls pass the value in `location.state`; the fourth (`OrderChangeEditStopsRoute`'s cancel-back path) would have regressed that one route alone. The pre-existing lossy-on-refresh caveat stands — the detail DTO carries neither identifier.
- **Source:** user ruling 2026-09-15.

### DEC-149: the grid sorts on the SEQUENCE, prefix-blind — the alphabet must not fake a consolidation grouping
- **Previous:** DEC-148 made the Odyssey Shipment Identifier the default sort, mapped to the raw text column. Sorting text alphabetically blocks every `C…` ahead of every `O…`.
- **Decision (user, 2026-09-15):** `SORT_MAP.odysseyShipmentIdentifier` is the expression `substr(odyssey_shipment_id, 2)::bigint`, so rows order by creation sequence and the two prefixes interleave. The reason is forward-looking: **consolidation — grouping shipments together — is being built**, and a default grid that splits C from O would read as that feature already working. Grouping by consolidation must be an explicit feature, not an alphabet artifact. The cast also retires the sort half of Dave's "do NOT make assumptions on length" — lexicographic order breaks the day the sequence reaches 9 digits, numeric order does not.
- **Source:** user ruling 2026-09-15; verified live (asc `O50000000 O50000001 O50000002 C50000003 O50000004`).


### DEC-150: "Consolidation" is a TYPE, not an order count — the dropped-back cohort is seeded
- **Previous:** the generator derived both the prefix and `shipmentType` from `orderCount` at creation, so a `C` shipment holding one order was unreachable — 0 of 10,000. Dave Schultz says that state is real: *"once a shipment is a consolidation, even if it goes back down to one order, it still has to be treated as a Consolidation… once something is a 'C', it stays that way forever"* (NN integration forces it).
- **Decision (S148):** a deterministic cohort — single-order shipments where `seqAtCreation % 50 === 0` — keep a `C` prefix and `shipmentType: 'Consolidation'` while holding one order. **76 of 10,000** in Neon. Picked off the sequence counter, never a faker draw: verified against git HEAD that `buyShipment`, `sellShipment`, `orders`, `pro`, `customerId`, `scac`, dates and weights are byte-identical on all 2,200 mock rows, the only diffs being the new field and 23 `shipmentType` flips. `isConsolidation` at `generate.mjs:1663` stays keyed to `orderCount > 1` — a dropped-back shipment's stops, weights and costs legitimately describe its one remaining order, and coupling it would fabricate a multi-order story.
- **The invariant that replaces the old one:** prefix ⟺ `shipmentType` is ABSOLUTE. "`C` implies more than one order" is retired and must never be reintroduced — deriving the prefix from a live order count would break Dave's NN rule silently.
- **Flag:** an older **LINX-11597** test encoded "Direct ⟺ exactly one order" and had to be updated. If 11597's AC states that as a hard rule, Dave's operational rule supersedes it — worth confirming with him or Jana, since a Jira AC normally outranks inference here.
- **Source:** Dave Schultz, 2026-09-15; user ruling to seed it.

### DEC-151: the identifier rides in the detail payload, so the Order Change header survives a refresh
- **Previous:** both Order Change routes read the shipment's name from `location.state`, so a hard refresh or pasted URL degraded the header to `Shipment 25969909` (the raw sell id). `OrderChangeReviewRoute.jsx`'s own comment recorded that the DTO carried only `shipmentId` and called fixing it out of scope.
- **Decision (S148):** `odysseyShipmentIdentifier` is added to the generated `detail` payload (the SellShipmentOut shape), carried through `SellShipmentOut` → `mapSellShipmentOutToDetail` → `ShipmentDetailVM`, and the routes prefer the FETCHED value over nav state (fetched → state → `buyShipment` → sell id). `location.state` is still passed so the header is right on first paint, before the fetch resolves. An absent field maps to `''` rather than `'--'` so the fallback chain still falls through instead of rendering `Shipment --`. Verified live: `GET /shipment-service/v1/sell-shipment-out/25969909` returns `odysseyShipmentIdentifier: "C50000000"`, and all 10,000 rows' `detail` jsonb matches their column (0 mismatches).
- **Source:** user ruling 2026-09-15.

### DEC-152: the identifier reaches the table — a whitelist mapper, and a guard that named the wrong set
- **Previous:** DEC-144 through DEC-151 wired `odysseyShipmentIdentifier` through Neon, the `ROW_COLUMNS` projection, `search_index`, the column picker, the grid config, the search registry and five components. It rendered **blank on every one of them**, and the suite was green.
- **Cause (S149):** `mapShipmentErrorRow` — the DTO → view-model step every grid row passes through — is a whitelist that builds a new object, so an unnamed field is dropped between a correct API and a correct column definition. Third shipped instance of the class (Pickup #, then Shipment Type / Planning Type). Its docblock instructs "ADD EVERY NEW COLUMN HERE" and claims its test pins it against `ALL_COLUMNS`; the test in fact pinned only `LATE_ADDED_COLUMNS`, which is why S148 passed. **A guard that names the wrong set is worse than no guard — it buys false confidence.**
- **Decision:** the field is declared on `ShipmentErrorRow` and `ShipmentRowVM` and carried by the mapper; the pin becomes the **whole catalog** minus an explicit `NOT_IN_LIST_DTO` set naming the 21 picker columns the list endpoint genuinely does not send. Adding a grid column is now four edits — DTO type, VM type, mapper, `LATE_ADDED_COLUMNS` — not the two it looked like.
- **Source:** defect found S149 while reviewing S148; confirmed in the live grid, bottom bar, details modal, Order Change header and live search.

### DEC-153: a late-added column lands at its catalog position, not appended
- **Previous:** `odysseyShipmentIdentifier` was never added to `LATE_ADDED_COLUMNS`. A persisted preset overrides the defaults wholesale, so the column was **absent** — not merely blank — for every user who had ever saved one, i.e. all of them. Confirmed against the live `user_preferences` row: none of the six saved presets contained the key.
- **Decision (S149):** the key joins `LATE_ADDED_COLUMNS`, and `mergeLateAddedColumns` now splices a missing column in at its `ALL_COLUMNS` position instead of appending it — appending is wrong for this one specifically, since it is the LEAD column by DEC-144 and the append buried it at the far right. The three default profiles are set-deduped (`withLateAdded`) because the identifier is now both placed explicitly and late-added.
- **Source:** defect found S149.

### DEC-154: the bar label is "Odyssey Shipment ID"; the field name is unchanged
- **Previous:** DEC-144 gave the attribute the label *Odyssey Shipment Identifier* — 27 characters, which ellipsized in every fixed-width surface it reached. Measured in the Details modal's identifier strip: the label lost **102px** of 192px and read `ODYSSEY SH…`.
- **Decision (user, 2026-09-16):** the human-facing label everywhere becomes **"Odyssey Shipment ID"** — grid header, column picker, details strip, search bar attribute and the live search registry (which `registryParity.test.js` pins to the client label). `dataKey` / column / DTO field stay `odysseyShipmentIdentifier`: the LLD's name is unchanged, only the label moved. Deliverables regenerated and `progression:audit` green in the same commit, per CLAUDE.md.
- **Residual, not fixed here:** the rename takes the modal strip's label clipping from 102px to 49px but does not clear it — that strip carries six cells in ~776px and **every** label in it ellipsizes (`BUY SHIPME…`, `PRO/BOOKIN…`, `RATING STAT…`, `SELL SHIPME…`), which predates this column. The cell geometry belongs to the normalized `SummaryStrip` master, so it is a Figma question, not a local override. `truncationTooltip` is enabled on that instance meanwhile.
- **Source:** user ruling 2026-09-16.

### DEC-155: Orders → Shipments is a row action that lands on a committed Order # search — offered only where the status means "in a shipment"
- **Previous:** the app had no path from an order to its shipment. Every shipment lists its orders (`orders[]`, the audit trail's "created successfully for Order …"), but the order row carries **no shipment reference at all** — the `orderList` DTO has no sell/buy id and no Odyssey Shipment ID — and the Orders UI had zero links into `/shipments`.
- **Decision (user, 2026-09-17):** a **"See in Shipments"** entry in the Orders row action menu (third, after View and Audit Trail). It navigates to `/shipments` with `state.orderNumber`; `ShipmentsRoute` turns that into an **Order # chip** (`attrChip('order', …)`, the same builder saved filters use) and hands it to the search bar as `seedChips`, which the hook takes as its **initial chip state** and commits to the table once on mount. The user lands on the Shipments page with the chip in the bar, the table already filtered, and the results glimpse closed.
- **Why a search, not a shipment id on the row:** (1) the order DTO has nothing to link on, and inventing a field the LLD does not carry would be seed-only truth; (2) a search returns EVERY shipment holding the order, which a multi-leg chain (migration 007, `legType` / `nextShipmentId`) can legitimately make more than one — a single id would silently pick one leg.
- **Which statuses:** only **Planned Shipment** and **Shipment Failed**, the two whose meaning in the LINX-7555 status table is "part of a shipment". *Ready for Planning* is the entry status (no shipment exists yet — the shipment is born in Planning & Consolidation, which is what takes an order from Planned Load to Planned Shipment); *Planned Load*, *Planning Failed*, *Hold*, *Draft* and *Cancelled* are excluded. **Seed-vs-canon note:** the generator puts every Planned Load order in a shipment (541/541 — it never models the pre-shipment load stage), so the link would "work" there; the canon wins and the gate stays narrow until Jana confirms whether a Planned Load has a shipment to show ([[feedback_seed_data_is_not_a_source]]).
- **Implementation note:** the first attempt applied the chips after mount behind the bar's `dismissRef` guard; React StrictMode's double-invoked mount effects consumed the guard on the empty bar and the glimpse opened anyway in dev. Seeding the hook's initial state (`initialChips`) has no transition to suppress, which is why it is the mechanism.
- **Source:** user ask 2026-09-17; verified in the browser against the live DB (KEM-91254, Shipment Failed → `/shipments`, chip `Order #: KEM-91254`, 1 item, glimpse closed).

### DEC-156: An order's creation creates its direct shipment
- **Previous:** `createOrder` (mock + live) wrote the order at `Ready for Planning` and touched no shipment; orders created in a session never appeared in Shipments at all. `orders.shipment_sell_id` existed in the schema and was filled only by the seed.
- **Decision (Dave Schultz + user ruling, 2026-09-17):** creating a valid non-draft order also creates ONE direct shipment holding that order's load, and the order moves straight to `Planned Shipment` (`Planned Load` is a transient nobody sees). Identifiers derive from the order's serial id, in bands disjoint from the seed: Odyssey Shipment Identifier `O` + (60,000,000 + orderId), sell `26,000,000 + orderId`, buy `900,000,000 + orderId`, load = orderId (order ↔ load is 1:1). The shipment is parked in `Monitoring › Consolidation` (the optimization pool) when the order is Consolidatable, `Monitoring › Hold` otherwise. No tender state is ever written and nothing auto-tenders. One pure builder, `api/_lib/planShipment.mjs`, serves both runtimes — mock writes to an in-memory overlay, live writes `shipments` + the `orders` link + `search_index`.
- **Rationale:** Dave Schultz (designed the legacy TMS; highest authority on the shipments domain) on the 2026-09-17 call: a load is always in a shipment from the moment it exists; every new shipment is created in the optimization pool, Hold, or Review — never tendering; and the system never auto-re-tenders. Manual consolidation (Oct MVP) is built FROM these direct shipments, so the pool has to fill when orders are created or the Candidate Workbench has nothing to select.
- **Source:** `vault/00-inbox/Planning and Consolidation.vtt` (Dave Schultz + Adam Shingle + Manuela, 2026-09-17) at 00:02:02 (load always in a shipment), 00:21:30 (the three creation states), 00:24:25 (a consolidation returns to the pool), 00:17:30 (never auto-re-tenders); identifier scheme per Dave Schultz 2026-09-15 (S148). **Flag: that VTT is still in `vault/00-inbox/` — the `/analyze` archive step to `vault-sources/` has not run yet.**
- **Affects:** `api/_lib/planShipment.mjs` (new), `api/_lib/orders.mjs` `createOrder`, `src/api/services/orderService.ts` `createOrder`, `src/api/services/shipmentService.ts`, `src/data/index.js` (shipments overlay), `src/search/shipments/searchIndex.js`, `src/api/queries/useCreateOrder.ts`, `src/api/types/createOrder.ts`.

### DEC-157: Draft submit and the Review creation state are deliberately NOT planned yet
- **Previous:** not addressed prior to this session — no ruling existed for draft submit's shipment creation, the Review state, or pool-exit timers.
- **Decision:** `submitDraftOrder` still moves an order to `Ready for Planning` without creating a shipment. The third creation state Dave named — **Review**, reached when no carrier list can be built — is not modelled at all. Timers (the ANC advanced-notification cutoff that moves a shipment out of the pool into tendering) are also not modelled.
- **Rationale:** scope was held to "create an order → a shipment exists" (user, 2026-09-17: *"we create an order, a shipment is created with that order in it, period"*). Review depends on a routing call this prototype does not make, and Dave placed the timers squarely in the backend (*"that's not UI logic"*) — a timer that drained the pool would also actively fight the consolidation demo.
...
### DEC-160: the Consolidatable flag is the ORDER's; pool vs Hold is the SHIPMENT's, and they are not the same question
- **Previous (S151 first cut):** one value, `poolEligible = orderCount > 1 || p(0.70)`, served as both the order's Consolidatable flag and the shipment's tab. That made every multi-order shipment sit in the pool by construction, and the measured pool came out **262 Consolidation / 172 Direct — 60.4% already consolidated.** Dave ruled a from-scratch consolidation can only pick **direct** shipments, so most of the pool was unpickable by the feature the pool exists to feed.
- **Decision (user, 2026-09-18 — bring it to ~40%):** the two are split. `consolidatable` stays exactly as it was (the ORDER's flag: multi-order ⟹ true, else p(0.70)), and the orders still inherit it. Tab placement becomes `inPool = consolidatable && orderCount <= CONSOLIDATION_POOL_MAX_ORDERS` (2). An already-consolidated shipment waits in the pool only while it can still **take** loads — the other thing Dave said consolidation does — and a full one moves to Hold, which is what Hold is for ("not eligible / too early").
- **Result, measured:** pool **303 — 131 Consolidation / 172 Direct = 43.2%** (from 60.4%); Hold 189. Pinned by a test that fails if the share leaves 30–50% or if anything with >2 orders is left in the pool.
- **The proxy is ours and it is a placeholder.** "Has room" should be weight/volume against the equipment; this generator models neither, so order count stands in. `CONSOLIDATION_POOL_MAX_ORDERS` is the single knob. **Replace it when Dave gives us the capacity rule.**
- **Costs nothing in ids:** `inPool` adds no faker draw, so the whole corpus is byte-identical on sell/buy/Odyssey ids and order lists — verified, not assumed. Only `category` moves.
- **Still provisional on OCM.** Dave never named OCM for manual planning; revisit if Ramesh's OCM 97–101 turns out to gate it.
...
### DEC-169: a historical version never holds an Accepted tender — **SUPERSEDED by DEC-175**
...
- **Status:** ~~OURS, provisional, for Jana.~~ **Reversed 2026-09-18 (DEC-175)** — the premise was ours and the documented record contradicts it. The response-coupling half survives (DEC-174); only the "never Accepted" half falls.

### DEC-170: the orders on a version accumulate; the sections are the Tender tab's own — *second half amended by DEC-176: the sections keep the Tender tab's column GROUPS but not its repeated identity prefix, and Routing Options no longer carries the response columns*
...
### DEC-173: a tender "response" is the carrier answering, and `responseMethod` is how Odyssey RECORDED it
- **Previous:** `Response Method` was read loosely as the carrier's channel, and `Response User` as an unqualified person — the seed put a `faker.person.fullName()` on 100% of tendered rows.
- **Decision:** LINX-5921 splits the two directions — Odyssey **notifies** the carrier (*"tender messages must be sent to carriers using API, email, EDI and Manual"* → our `apiSource` / `notifyDateTime`), and the carrier **responds** back (*"carrier actions or system updates (user actions as well) modify tender outcomes"*). `RESPONSE_METHODS` is `['API Update', 'EDI Update', 'Manual Update', 'Automatic Update']` — update **mechanisms**, not carrier channels. So: API/EDI Update = the carrier's system wrote it (no user); Automatic Update = Odyssey itself on expiry/timeout (no user, no carrier); **Manual Update = an Odyssey person keyed in a phoned/emailed answer**. `responseUser` is therefore **ours**, and exists only on a Manual Update.
- **Consequence:** `Cancelled` is **our** action, not the carrier's — LINX-5921 lists Cancel among the actions *"the user should be able to perform"* — so a cancelled row can only have been recorded manually or automatically, never by the carrier's own feed.
- **Source:** LINX-5921 (Tender Process, the parent epic of 15895); user question, 2026-09-18.

### DEC-174: the four response fields are seeded as ONE fact, and `responseComments` becomes a real field
- **Previous:** three independent gates in `generate.mjs` described one event — `responseMethod`/`responseUser` on `wasTendered` but `responseDateTime` on `isAccepted`. Measured over 9,880 options: **5,210** Declined/Cancelled rows named a responder and a channel for a response with **no date**; **312** `Sent` rows named a responder for a response that had not happened; **4,583** API/EDI/Automatic rows named a person for an update **no person made**. Same defect class as S151's monitoring tabs. `responseComments` did not exist at all — `routingHistory.js` invented the text at read time (the gap DEC-171 recorded as owed).
- **Decision:** the four fields are derived from the outcome in one block — `Sent`/untendered record nothing; an answered row always has a date and a method; a user appears only on `Manual Update`; `Cancelled` can never be an API/EDI Update; and Declined/Cancelled always carry a comment, Accepted about a third. The comment pools move to `src/data/responseComments.js`, imported by **both** the generator and the derive, so a seeded row and a derived historical one cannot drift.
- **Cost:** **zero new faker draws** — the date reuses the pure `formatDateTime(genDate(...))`, the cancel remap and the comment pick are indexed by the already-drawn `lcePkId`, and `responseUser` keeps the roll and discards it (S151's accessorials trick). Verified: all 2,200 detail files byte-identical outside the four fields, so no id, order number, date, weight, carrier or status moved, and the SpotBid fixtures and golden ids survive. **Neon reseeded** on the user's explicit go, 2026-09-18 — no schema change, because live stores the option as a JSONB blob (`tenders.option`).
- **Owed:** the real `ShippingOption` carries **both** `responseReason` (a code) and `responseComments` (free text). We seed only the text; a reason-code vocabulary is a question for Jana, not a gap to fill silently.

### DEC-175: a historical routing version CAN hold an Accepted tender — re-routing after acceptance is the documented norm, not an impossible state
- **Previous:** DEC-169 — *"an accepted tender ends the routing story; that shipment would not have been re-routed."* Ours, provisional, and argued from nothing but the shape of our own seed.
- **What the documentation says.** Jana, Mar 25 grooming #2 (the definition of a version): *"The complete routing changed. The first set of carriers generated was not the same as the second set because there was a **new order added** or a **new stop** came in or the **origin changed**"* … *"The first set becomes a tender history and then the second one, third one, you can generate many more."* The Order Change stories are the mechanism, and they start from an accepted shipment: **LINX-14509** enters review from *To Be Tendered, Sent, **Accepted*** and *"routing and rating re-run … produce a **new Tender Option Version**"*; **LINX-14514** *Keep Carrier & Re-Tender* → *"Sent, from Sent **or Accepted** — an accepted carrier must re-accept"*, and *Bypass* → *"**Accepted stays Accepted**"* across the version change; **LINX-15438** re-routes automatically on non-location changes; **LINX-15435/15438** *"new list becomes **V2** on the Tender tab; **V1 = prior**."* **LINX-15899** (*Tender Actions and Tender Status Management at Option Level*, Final Review — per Rovo, 2026-09-18; **not yet in the vault**) permits Cancel / Decline / Re-Tender on an Accepted status — a second documented route. Carrier giveback (equipment, capacity, hours-of-service) is a third, from general TMS practice rather than a story.
- **Decision:** a historical version holds whatever its routing execution held, **Accepted included**. The AC's snapshot rule governs the contents — *"shall represent the snapshot of the data captured during that routing execution … shall not be updated when subsequent routing executions occur"* — so an accepted row's artifacts (Carrier Pickup #, Pro #, delivery #) stay on it in history. That is the column's operational purpose: a planner or CSR contacting a carrier about a cancelled or re-routed load cites the **historical** pickup number.
- **The salvageable half of DEC-169:** a shipment is never *active* in two versions at once. True, and different from what we wrote.
- **Consequence for the derive (spec `docs/superpowers/specs/2026-09-18-routing-history-accepted-in-history.md`):** the seed already carries a **genuine V1** for every order-change shipment — `orderChange.priorTenderList` / `newTenderList` / `droppedCarriers.prior` — with its real Accepted row. Those shipments get their true prior version, not a perturbation. Shipments with no documented re-route reason keep the Declined/Cancelled/Sent/none perturbation, and the perturbation must now **null the acceptance artifacts it was leaving behind** — measured 2026-09-18: 543 of 2,432 derived historical rows (22%) carried a Carrier Pickup # under a Sent/Declined/Cancelled status, e.g. `O50001096` V4 `FXFE` *Sent* with `VOF10221`. The defect is the rewritten status keeping the artifact, not the artifact.
- **What stays open:** what populates Carrier Pickup # in the first place is **undocumented** — the column name and one sample value (`SAA9999`) are all the sources hold; "on acceptance" (ours, from the seed gate) and "on the carrier's response" (S113's audit inference, DEC log line 455) are both unsourced. And S113's own question, unanswered since Aug 10: does a re-tender void the prior cycle's Pro # / Pickup # on the **live** row? Neither affects history, which snapshots.
- **Source:** documentation review 2026-09-18 (user: *"double check with everything you know about tender from documentation not implementation"*), cross-checked against Rovo's read of the live Jira the same day.

### DEC-176: the Routing History surface — what each section shows, and how the cards open (user rulings, 2026-09-18)
- **Previous:** S152 built each version as a SubAccordion holding five section tables lifted from `tenderColumns.js`, every one headed by the four locked identity columns (Route Rank · Rank · SCAC · Carrier Name), all sections collapsed, all versions collapsed, the section cards shadowed like their parent.
- **Decision 1 — each section shows only its own data.** Measured: 64 columns across the five tables, 20 identity, 16 pure repeats of the same four carrier rows, three tables scrolling sideways. The repetition was inherited, not designed — on the Tender screen those groups are **sub-tabs of one table**, which is what "locked" means: held still *while the tab changes*; stacked as accordions, the lock became duplication. Secondary sections now keep **SCAC alone** as the row anchor (a row must stay attributable; GroupTable's flat mode only falls back to `group.label` when the lead column is empty, so dropping it too would leave rows anonymous); Routing Options drops **Response Method / Date / User**, which have their own section here (the Tender screen hangs them off Routing Options only because it has no Response Comments tab). **64 → 49 columns**; Response Comments and Volume Commitment no longer scroll. `tenderColumns.js` is untouched — the live Tender tab's grouping is correct there. **Tender Status** is the one deliberate overlap: a response without its outcome is meaningless. *Amends DEC-170's second half.*
- **Decision 2 — the newest version opens on arrival**, the rest collapsed: it is the one directly behind what the Tender tab shows, so it is what the planner came to read; all-collapsed made the AC's *"expand and review each routing version independently"* a wall, all-open a scroll.
- **Decision 3 — the section cards are outlined, not shadowed, with 16px titles** (`--font-size-base`) against the version card's 18px, and a full-bleed rule separates the version header from its content (VD `2259:70166`). A shadow inside a shadowed parent reads as a smudge; the size step is what makes the nesting read as hierarchy. All scoped to the pane, no molecule change.
- **Story basis:** none of this is in LINX-15895. The AC names the five sections and defers everything else — *"Refer to the View Design for layout, expand/collapse behavior, section organization, and version presentation details."* The VD is the designer's own, so these are design rulings, not deviations.
- **Found underneath, design-system, not ours to own here:** nested SubAccordions had never collapsed. `components.css` matched both state rules with a **descendant** combinator (`.sub-accordion--expanded .sub-accordion__reveal` / `…__chevron`), so an expanded card forced every card nested inside it open with an up-chevron — browser-measured: five sections at `aria-expanded="false"` yet 264px tall, while the same component collapsed to 0px on the DSM where nothing nests it. Scoped both with `>`; no consumer changes, only nesting behaves differently; pinned by a structural test since jsdom sees no layout. **The Angular twin mirrors this CSS and is mid-port at GATE C-twin — it almost certainly carries the same bug.** Owed to the D-thread.
- **Source:** user, 2026-09-18 (*"remove Route Rank / Rank / SCAC / Carrier Name or/and only show the relevant per subaccordion"*; *"top version (latest) should be expanded by default"*; *"inner subaccordions should come with an external border line … slight smaller font size 16px"*). Commits `5f93079`, `9d793ed`.
...
| Sep 18, 2026 | DEC-160 amended (S151 follow-up) — the **optimization pool is made pickable**: Consolidatable (the order's flag) and pool-vs-Hold (the shipment's tab) are separated, so an already-consolidated shipment waits in the pool only while it can still take loads; the pool goes from **60.4% to 43.2% consolidated** with no faker draw and therefore no id movement, pinned by a test |
...
| Sep 15, 2026 | DEC-150 + DEC-151 — **Consolidation is a type, not a count**: a seeded dropped-back cohort (76/10,000 `C` shipments holding one order) makes Dave's "once a C, always a C" state reachable, retiring the "`C` implies >1 order" assumption and leaving prefix ⟺ `shipmentType` as the absolute invariant; and the identifier now rides in the detail payload so the Order Change header survives a hard refresh |
| Sep 15, 2026 | DEC-148 + DEC-149 — the identifier becomes the **displayed name** on all five remaining surfaces (default sort, both Order Change headers, bottom panel, mock + live search rows, with `hydrate.columns` widened) while keying stays on `sellShipment`; and the grid sorts **prefix-blind** on `substr(odyssey_shipment_id, 2)::bigint` so the alphabet cannot fake a consolidation grouping ahead of the real feature |
| Sep 15, 2026 | DEC-144 through DEC-147 — the **Odyssey Shipment Identifier** (Laurie + Dave Schultz): a new lead grid column and lead search attribute, one ID common to the shipment's buy and sell side, `O` single-order / `C` consolidated + a sequence from 50,000,000 with deliberately unbounded length; amends DEC-66 by demoting Buy/Sell Shipment # to internal-only while keeping `sellShipment` as the key, row id and detail link; `match: 'both'` because the letter prefix would otherwise make the lead identifier unsearchable; generated by a counter not a faker draw, all 2,200 existing ids verified unchanged |

...
- **Why it is a library slot and not a one-off:** the pattern is "one long free-text field among short ones", which is not specific to dropped carriers. The consumer supplies the node; GroupTable only owns the row.

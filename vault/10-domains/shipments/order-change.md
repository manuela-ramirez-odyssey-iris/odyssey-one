...
- **OC-open-22** — The 15872 move updates `shipments.detail` (orderList, stops), `orders` and `order_count` on target and source, but NOT the `orders` table's `shipment_sell_id` (still the source) nor the shipment rows' list aggregates (`gross_weight` etc.). The Orders grid and the Shipments list can disagree with the detail after a move. Seam to close when the move gets a real backend (or at the next reseed design).

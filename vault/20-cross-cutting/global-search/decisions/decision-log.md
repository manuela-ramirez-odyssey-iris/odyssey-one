...
**Decided:** 2026-08-05 · **Corrected:** 2026-08-05 (see below) · **Status: INVENTED — needs Jana**
...
**⚠ CORRECTED 2026-08-05 — the first two defaults were WRONG and returned zero results.** Original text claimed Mode and Tender Status were "verified projected". That verification checked the **client-side** `SHIPMENTS_PROGRESSION` and the **mock** adapter. Live mode routes through a *different, smaller* **server-side** registry (`api/_lib/search-registry.mjs`), which contains neither key; `validChips` silently drops unknown keys and `buildHits` falls to its honest-empty `WHERE FALSE` branch. Since `.env.local` sets `VITE_API_MODE=live`, the mock path was never what the app ran. Both defaults shipped broken and were replaced.

**The rule this establishes:** a saved-filter attribute must exist in **BOTH** vocabularies — the client progression (which builds the chip) and the server registry (which matches it). Checking one is not checking the other. Written into the constant's comment in code so it can't be repeated.

**Replacement defaults (2026-08-05):** **"SEFL Carrier"** (`scac` = SEFL) and **"Ex-Houston"** (`origin` = Houston) — both keys present in both vocabularies, and verified **read-only against the live `search_index`: 680 and 324 rows respectively**. Tests now assert both that each default's key is in the server registry and that each returns a non-zero count, so a future reseed that invalidates them fails the suite rather than shipping silently broken.

**Rationale:** No canon defines what Odyssey's shipped defaults should be — these remain our invention. The attribute choice is now constrained by what live search can actually match, which is a smaller set than the filter panel exposes. **Mode/Tender Status defaults are still the ones we'd want semantically** — they need `mode` and `tender-status` projected into `search_index`, which is DB work and stays gated (user chose "rebuild on indexed attributes now", 2026-08-05).
...
### GS-30 — The progression sheets are GENERATED, and the two domains ship as siblings
**Decided:** 2026-09-11
**Previous state:** Two hand-maintained artifacts that had drifted apart and away from the build. The Shipments sheet was the 2026-05 stakeholder grouping, never reconciled: `Pickup #` still marked *skipped* four months after it shipped, `Order Count` and the whole Classification group (Shipment Type, Planning Type) absent, and 22 proposed attributes reading as though they were implemented. The Orders sheet listed 7 Order Status values with wrong wording against the code's 8. There was no Shipments markdown doc at all — only Orders had one — so the pair could not be handed over as a set.
**Decision:** Both taxonomy sheets and both CSVs are generated from the code by `npm run progression:sheets`, which reads `search/shipments/progression.js`, `search/orders/progression.js` and `search/orders/registry.js` through Vite's resolver. `npm run progression:audit` re-renders and diffs, exiting 1 on drift. The two workbooks share one seven-sheet shape and one style function, so they are siblings by construction rather than by discipline. The two markdown docs share one ten-section skeleton. Attributes that were proposed and never built are NOT deleted — they move to a `Not Implemented` sheet with a colour-coded `Status`, so the stakeholder proposal survives without masquerading as a spec.
**Rationale:** User AC: *"every time we do updates to this search functionality we need to make sure the xls are aligned in case we modified something in vercel. The same for md files of each."* A convention cannot deliver that; a failing command can. The sheets go to Cognizant as the search specification, so a stale sheet is a wrong spec, not a stale note — and the Shipments sheet had been wrong for four months without anyone noticing, which is the evidence that hand-maintenance does not hold.
**Also settled:** the sheets now carry the group label sets as SEPARATE columns, because the surfaces genuinely disagree and nobody could tell which label was which. The filters panel section header renders the group NAME (`group.group`); the suggestions panel section title renders the group's DRILL LABEL (`group.label`). Shipments has two such label sets because its panel is generated from the progression; Orders has three, because its filters panel has its own five registry group names (*Order Identifiers · Route & Geography · Schedule · Audit Trail · Draft & Errors*) that do not match the bar's nine. That divergence is recorded as an open question for Ramesh, NOT reconciled in code.
**Source:** User, 2026-09-11 (S146), four acceptance criteria. Code truth verified in-session against the modules: Shipments 27 attributes / 10 groups, Orders 20 attributes / 9 groups, Orders panel 13 fields.
**Affects:** `tools/progression-sheets.mjs`, `tools/progression-sheets.py`, `docs/progression/prose.json`, `docs/story-packs/{shipments,orders}-search-progression.xlsx`, `vault/10-domains/{shipments,orders}/data/attributes-progression-grouping.csv`, `vault/10-domains/shipments/shipments-search-progression.md` (new), `vault/10-domains/orders/orders-search-progression.md`, both domain MOCs, `CLAUDE.md`.

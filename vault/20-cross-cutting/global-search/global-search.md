...
- ~~**Saved filter as single chip** (GS-09)~~ — **SUPERSEDED 2026-08-05 by [[decisions/decision-log#GS-24 — Saved filters store CHIP OBJECTS and apply as the real chip set (supersedes GS-09 + GS-10)|GS-24]].** Applying a saved filter restores the **actual chip set**, not one collapsed named chip — the bar must stay "the reflection of what is in the search bar" (user, 2026-08-04), and an opaque chip can be neither edited nor partially removed.
